...
export function StatTile(props: StatTileProps) {
    ...
    return (
        <div
            class={["happy2-stat-tile", local.class].filter(Boolean).join(" ")}
            data-happy2-ui="stat-tile"
            data-testid={local["data-testid"]}
            data-tone={tone()}
            style={local.style}
        >
            <div class="happy2-stat-tile__header" data-happy2-ui="stat-tile-header">
                <span class="happy2-stat-tile__label" data-happy2-ui="stat-tile-label">
                    {local.label}
                </span>
                <Show when={local.icon}>
                    {(name) => (
                        <span class="happy2-stat-tile__icon" data-happy2-ui="stat-tile-icon">
            ...
            <div class="happy2-stat-tile__value" data-happy2-ui="stat-tile-value">
                {local.value}
            </div>

            <Show when={hasFooter()}>
                <div class="happy2-stat-tile__footer" data-happy2-ui="stat-tile-footer">
                    <Show when={local.delta}>
                        {(delta) => (
                            <span
                                class="happy2-stat-tile__delta"
                                data-happy2-ui="stat-tile-delta"
                                data-trend={delta().trend}
                            >
                                <svg
                                    aria-hidden="true"
                                    class="happy2-stat-tile__delta-arrow"
                                ...
                                <span
                                    class="happy2-stat-tile__delta-value"
                    ...
                    <Show when={local.hint}>
                        <span class="happy2-stat-tile__hint" data-happy2-ui="stat-tile-hint">
                            {local.hint}
                        </span>
                    </Show>
                </div>
