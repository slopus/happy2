 ...
 * Presentational and fully controlled; the consumer owns the delete request
 * and supplies pending/failure state.
 */
export function DocumentDeleteDialog(props: DocumentDeleteDialogProps) {
    ...
    return (
        <Modal
            {...rest}
            className={["happy2-document-delete-dialog", local.className].filter(Boolean).join(" ")}
            footer={
        ...
        >
            <Box className="happy2-document-delete-dialog__body">
                {local.error ? (
                    <Banner data-testid="document-delete-error" tone="danger" title="Delete failed">
                        {local.error}
                    </Banner>
                ) : null}
                <span
