...
export function Button(props: ButtonProps) {
    ...
    return (
        <button
            {...rest}
            class={["happy2-button", local.class].filter(Boolean).join(" ")}
        ...
        >
            <span class="happy2-button__content" data-happy2-ui="button-content">
                <Show when={local.icon}>
                    {(name) => (
                        <span class="happy2-button__icon" data-happy2-ui="button-icon">
                            <Icon name={name()} size={iconSizes[size()]} />
                        </span>
                    )}
                </Show>
                <Show when={!local.iconOnly}>
                    <span class="happy2-button__label" data-happy2-ui="button-label">
                        {local.children}
                    </span>
                </Show>
            </span>
