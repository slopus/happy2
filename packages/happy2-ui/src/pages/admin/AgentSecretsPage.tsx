        ...
        <StoreSurface store={props.store}>
            {(snapshot, store) => {
                ...
                const openSecret = createMemo(() =>
                    secrets().find((secret) => secret.id === detailId()),
                );
                const agentById = createMemo(
                    () => new Map(snapshot().agents.map((user) => [user.id, user])),
                );
                const channelById = createMemo(
                    () => new Map(snapshot().channels.map((chat) => [chat.id, chat])),
                );
                const agentBindings = createMemo<AgentSecretBinding[]>(() =>
                    (openSecret()?.agentUserIds ?? []).map((id) => {
                ...
                const attachedAgents = createMemo(() => new Set(openSecret()?.agentUserIds ?? []));
                const attachedChannels = createMemo(() => new Set(openSecret()?.channelIds ?? []));
                const actionError = createMemo(() =>
                    snapshot().actionError === dismissedError()
                        ? undefined
                        : snapshot().actionError?.message,
                );
                const secretsError = createMemo(() => {
                    const state = snapshot().secrets;
                                ...
                                <ModalOverlay onDismiss={() => setDetailId(undefined)}>
                                    ...
                                    >
                                        <AgentSecretDetail
                                            ...
                                            availableAgents={snapshot()
                                                .agents.filter(
                                                    (user) => !attachedAgents().has(user.id),
                                                )
                                                .map((user) => ({
                                                    value: user.id,
                                            ...
                                            availableChannels={snapshot()
                                                .channels.filter(
                                                    (chat) => !attachedChannels().has(chat.id),
                                                )
                                                .map((chat) => ({
                                                    value: chat.id,
                                                    label: chat.slug
