...
describe("channel workspace files", () => {
    it("lists Trees paths and Git changes only for current channel members", async () => {
        const root = await mkdtemp(join(tmpdir(), "happy2-channel-workspaces-"));
        try {
            ...
            expect(
                (await asOwner.get(`/v0/chats/${direct.json().chat.id as string}/workspace`))
                    .statusCode,
            ).toBe(404);
            ...
            const workspaceDirectory = join(root, "channels", chatId);
            ...
            const memberSnapshot = await waitForWorkspace(
                server.as(member),
                endpoint,
                (candidate) => !candidate.gitStatusPending && candidate.gitStatus.length === 7,
            );
            expect(memberSnapshot).toMatchObject({
                paths: snapshot.paths,
                gitStatus: snapshot.gitStatus,
                unloadedDirectories: snapshot.unloadedDirectories,
            });
            expect(
                (await asOwner.post(`/v0/chats/${chatId}/removeMember`, { userId: member.id }))
                    .statusCode,
            ).toBe(200);
            expect((await server.as(member).get(endpoint)).statusCode).toBe(404);
            ...
            expect((await server.as(outsider).post(`/v0/chats/${publicId}/join`)).statusCode).toBe(
                200,
            );
            expect((await server.as(outsider).get(publicEndpoint)).statusCode).toBe(200);
        } finally {
    ...
    it("adaptively defers expensive folders and pages their children with stale-cursor safety", async () => {
        const root = await mkdtemp(join(tmpdir(), "happy2-adaptive-channel-workspace-"));
        try {
            await using server = await createGymServer({
                configure(config) {
                    config.agents.defaultCwd = root;
            ...
            expect((await asOwner.get(`${endpoint}?directory=generated&limit=2`)).statusCode).toBe(
                404,
            );
    ...
    it("invalidates an open channel over SSE immediately after its workspace changes", async () => {
        const root = await mkdtemp(join(tmpdir(), "happy2-live-channel-workspace-"));
        try {
            // A repository containing the workspace root must not become the channel's repo.
            ...
            const member = await server.createUser({ username: "live_workspace_member" });
            const asOwner = server.as(owner);
            const channel = await asOwner.post("/v0/chats/createChannel", {
                kind: "private_channel",
                name: "Live workspace",
            ...
            for (let index = 0; index < 8; index += 1) {
                const extra = await asOwner.post("/v0/chats/createChannel", {
                    kind: "private_channel",
                    name: `Extra workspace ${index}`,
                    slug: `extra-workspace-${index}`,
                });
                const extraEndpoint = `/v0/chats/${extra.json().chat.id as string}/workspace`;
                await waitForWorkspace(
                    asOwner,
                    extraEndpoint,
                    (candidate) => !candidate.gitStatusPending,
            ...
            await writeFile(
                join(root, "channels", chatId, "live.ts"),
                "export const live = true;\n",
            );
            const changed = await frames.until((frame) => frame.name === "workspace.changed");
            expect(changed.data).toMatchObject({
                type: "workspace.changed",
                chatId,
                occurredAt: expect.any(Number),
            });
            const reconciled = await waitForWorkspace(server.as(member), endpoint, (candidate) =>
