import { storeCreate, type StoreWriter } from "../../kernel/store.js";
...
export interface SettingsStoreBinding {
    readonly store: SettingsStore;
    settingsInput(event: SettingsInput): void;
    avatarRevisionGet(): number;
    dispose(): void;
}
...
export function settingsStoreCreateBinding(
    options: SettingsStoreOptions = {},
    output: (event: SettingsOutput) => void = () => undefined,
): SettingsStoreBinding {
    ...
    const { store: readonlyStore, writer } = storeCreate<SettingsSnapshot>({
    ...
    const store: SettingsStore = {
        ...
        displayNameUpdate(firstName, lastName): void {
            local({ type: "displayNameUpdated" }, (snapshot) =>
                snapshot.profile.firstName === firstName && snapshot.profile.lastName === lastName
                    ? snapshot
                    : {
                          ...snapshot,
    ...
    return {
        ...
        settingsInput: (event) => {
            if (disposed) return;
            if (event.type === "avatarSaved") avatarRevision += 1;
            settingsInputApply(writer, event, avatarRevision);
        },
        avatarRevisionGet: () => avatarRevision,
        dispose(): void {
            if (disposed) return;
            disposed = true;
            writer.dispose();
...
function settingsInputApply(
    writer: StoreWriter<SettingsSnapshot>,
    event: SettingsInput,
        ...
        switch (event.type) {
            ...
            case "profileSaveFailed":
                return {
                    ...
                    fields: {
                        ...snapshot.fields,
                        displayName: fieldFailed(snapshot.fields.displayName, event.error),
            ...
            case "presenceSaveFailed":
                return {
                    ...
                    fields: {
                        ...snapshot.fields,
                        availability: fieldFailed(snapshot.fields.availability, event.error),
                        statusText: fieldFailed(snapshot.fields.statusText, event.error),
                        statusEmoji: fieldFailed(snapshot.fields.statusEmoji, event.error),
                        statusExpiry: fieldFailed(snapshot.fields.statusExpiry, event.error),
            ...
            case "notificationsSaved":
                return notificationsSaved(snapshot, event.notifications, event.submitted);
