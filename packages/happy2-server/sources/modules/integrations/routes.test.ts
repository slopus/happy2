...
import type { SecretProtector } from "./secrets.js";
import type { WebhookUrlPolicy } from "./ssrf.js";
import type { IntegrationChange, IntegrationRouteCallbacks } from "./types.js";

vi.mock("../bot/botCreate.js", () => ({ botCreate: vi.fn() }));
vi.mock("../integration/apiCredentialCreate.js", () => ({ apiCredentialCreate: vi.fn() }));
vi.mock("../webhook/incomingWebhookInvoke.js", () => ({ incomingWebhookInvoke: vi.fn() }));

const executor = {} as DrizzleExecutor;
const secretProtector = {} as SecretProtector;
const webhookUrlPolicy = {} as WebhookUrlPolicy;
...
describe("integration HTTP routes", () => {
    ...
    it("requires a product user and maps admin creation results", async () => {
        ...
        vi.mocked(botCreate).mockResolvedValue({
            value: {
                id: "bot_1",
                name: "Build Bot",
                username: "build_bot",
        ...
        const app = buildRouteServer({ incomingWebhook: sink(), onChange });
        ...
        expect(botCreate).toHaveBeenCalledWith(executor, {
            actorUserId: "admin_1",
            name: "Build Bot",
            username: "build_bot",
    ...
    it("returns credentials once and rejects unexpected fields", async () => {
        vi.mocked(apiCredentialCreate).mockResolvedValue({
            credential: {
                id: "credential_1",
        ...
        expect(apiCredentialCreate).toHaveBeenCalledTimes(1);
    ...
    it("accepts incoming webhook secrets only in the dedicated header", async () => {
        ...
        vi.mocked(incomingWebhookInvoke).mockImplementation(
            async (
                _executor,
                token: string,
                text: string,
        ...
        const app = buildRouteServer({ incomingWebhook });
        ...
        expect(incomingWebhookInvoke).toHaveBeenCalledWith(
            executor,
            "happy2_hook_secret",
            "hello",
            incomingWebhook,
...
function buildRouteServer(callbacks: IntegrationRouteCallbacks = { incomingWebhook: sink() }) {
