...
import { createFakeServer, jsonResponse, type FakeServer } from "rigged-state/testing";
...
function image(input: ImageInput) {
    return {
        ...
        dockerTag: `rigged-agent:${input.id}`,
        status: input.status,
        buildAttempt: 1,
...
function detail(
    ...
    return {
        ...image(input),
        dockerfile: extra.dockerfile ?? "FROM rigged/agent-base:latest",
...
describe("AgentImagesView", () => {
    ...
    it("creates a new image from a dockerfile and closes the dialog", async () => {
        ...
        const dockerfileField = view.getByPlaceholderText(/^FROM rigged\/agent-base/);
        fireEvent.input(dockerfileField, {
            target: { value: "FROM rigged/agent-base:latest\nRUN true" },
        });
        ...
        expect(createRequest?.body).toEqual({
            name: "Python + Node",
            dockerfile: "FROM rigged/agent-base:latest\nRUN true",
        });
        state.stop();
    });
    ...
    it("opens the detail dialog with the Dockerfile and build log on row click", async () => {
        ...
        expect(view.container.querySelector('[data-rigged-ui="modal-title"]')?.textContent).toBe(
            "Rust nightly",
        );
...
function rowFor(view: ReturnType<typeof render>, text: string): HTMLElement {
    ...
    const row = cell.closest('[data-rigged-ui="data-table-row"]');
