...
describe("App", () => {
  ...
  it("captures a human decision as agent context", () => {
    ...
    fireEvent.click(getByRole("button", { name: "View Default workspace names decision details" }));
    expect(getByText("Preserve existing saved names")).toBeTruthy();
    fireEvent.click(getByRole("button", { name: "Add Default workspace names decision to context" }));

    expect(getByLabelText("Attached context")).toBeTruthy();
    expect(getByRole("button", { name: "Default workspace names decision added to context" })).toBeTruthy();
  });
