    ...
    >
      <div class="flex items-center gap-2.5 px-3 py-2.5">
        ...
        <div class="min-w-0 flex-1">
          <div class="flex min-w-0 items-center gap-2">
            <h4 class="truncate text-[0.72rem] font-extrabold text-[#352e35]">{props.run.title}</h4>
            <span class={`shrink-0 rounded-full px-2 py-0.5 text-[0.56rem] font-extrabold ${statusStyles[status()]}`}>
              {statusLabel()}
            </span>
          </div>
          <p class="mt-0.5 truncate font-mono text-[0.57rem] text-[#8a8189]">{props.run.branch}</p>
        </div>
        <button
      ...
      <div class="px-3 pb-2.5">
        ...
        <p class="mt-1.5 text-[0.58rem] font-medium text-[#8c838b]">
          {finishedSteps()} of {props.run.steps.length} steps · {props.run.files.length} {props.run.files.length === 1 ? "file" : "files"}
        </p>
      </div>
            ...
            {props.run.steps.map((step) => (
              <li class="flex items-center gap-2 text-[0.65rem] text-[#574f56]">
                <span
                  ...
                  aria-hidden="true"
                >
                  {step.status === "done" ? "✓" : step.status === "working" ? "↻" : "·"}
                </span>
                <span class={step.status === "pending" ? "text-[#938b92]" : ""}>{step.label}</span>
              </li>
            ))}
          </ol>
