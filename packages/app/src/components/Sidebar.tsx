...
function UtilityIcon(props: { name: "compose" | "directory" | "headphones" | "invite" | "search" | "settings" }) {
...
function SectionIcon(props: { name: SidebarSectionIcon }) {
  if (props.name === "channels") return <span class="text-[1rem] font-semibold leading-none">#</span>;
  ...
  const path = {
    ...
    messages: "M7.5 17.5H5a2 2 0 0 1-2-2v-7a2 2 0 0 1 2-2h10a2 2 0 0 1 2 2v.8M10 13.5h9a2 2 0 0 1 2 2v3a2 2 0 0 1-2 2h-4.5L11 22v-1.5h-1a2 2 0 0 1-2-2v-3a2 2 0 0 1 2-2Z",
...
export function Sidebar(props: SidebarProps) {
  ...
  return (
    <aside class="flex h-full min-h-0 flex-col bg-[#f7f5fb] text-[#40364e]" aria-label={`${props.workspaceName} sidebar`}>
      <header class="flex h-[58px] shrink-0 items-center gap-1.5 border-b border-[#dfdbe7] px-3">
        ...
        {(["settings", "invite", "compose"] as const).map((action) => (
          <button
            ...
            type="button"
            aria-label={action === "settings" ? "Workspace settings" : action === "invite" ? "Invite people" : "New message"}
          >
      ...
      <div class="min-h-0 flex-1 overflow-y-auto px-3 pb-5 pt-3">
        <div class="flex h-[46px] items-center gap-2.5 rounded-[9px] border border-[#ddd7e7] bg-[#fbfaff] px-3 shadow-[0_1px_2px_rgb(49_34_58_/_3%)]">
          <span class="grid h-7 w-7 place-items-center rounded-full border border-[#d5cee0] text-[#5d4a72]" aria-hidden="true">
            <SectionIcon name="apps" />
          </span>
          <span class="min-w-0 flex-1 truncate text-[0.75rem] font-bold text-[#59476c]">Set up your space</span>
          <span class="h-6 w-6 rounded-full bg-[conic-gradient(#57d17d_0_26%,#ded8e5_26%)] p-[4px]" aria-hidden="true">
          ...
          {visibleSections().map((section) => (
            <section aria-labelledby={`sidebar-${section.id}`}>
              ...
              {section.emptyText && section.items.length === 0 && !query() && (
                <p class="mb-1 pl-7 pr-2 text-[0.65rem] leading-4 text-[#8d8398]">{section.emptyText}</p>
              )}
              ...
              <div class="flex flex-col">
                {section.items.map((item) => {
                  ...
                  return (
                    ...
                    >
                      {item.kind === "channel" && <span class="w-4 text-center text-[0.9rem] leading-none">#</span>}
                      {item.kind === "action" && <span class="grid h-4 w-4 place-items-center rounded bg-white text-[0.9rem] leading-none">+</span>}
                      {(item.kind === "person" || item.kind === "app") && (
                        <Avatar
                          backgroundClass={item.avatarClass ?? "bg-[#765c95]"}
      ...
      <footer class="shrink-0 border-t border-[#ded9e5] p-3">
        <p class="mb-2 text-[0.67rem] leading-4 text-[#71647e]">Rigged works better with your whole team.</p>
        <button class="flex h-8 w-full items-center justify-center gap-2 rounded-[6px] border border-[#d6d0dc] bg-white text-[0.69rem] font-bold text-[#4d4258] hover:bg-[#f1edf5] focus-visible:outline-2 focus-visible:outline-[#6f4b92]" type="button">
