  ...
  return (
    ...
    >
      <div class="border-b border-[#e7e2e8] bg-[#faf8fb] px-3 py-2">
        ...
        <p class="mt-0.5 text-[0.58rem] text-[#8d848c]">Give agents the source material behind your request.</p>
      </div>
      ...
      <div class="p-1.5">
        {props.items.map((item) => (
          ...
          >
            <span class={`grid h-8 w-8 shrink-0 place-items-center rounded-[8px] ${item.kind === "file" ? "bg-[#e8eef8] text-[#426995]" : item.kind === "thread" ? "bg-[#f1e7f3] text-[#79477f]" : "bg-[#e5f2e9] text-[#39764c]"}`}>
              <ContextIcon kind={item.kind} />
            </span>
            <span class="min-w-0 flex-1">
              <span class="block truncate text-[0.69rem] font-extrabold text-[#3b333b]">{item.label}</span>
              <span class="mt-0.5 block truncate text-[0.58rem] text-[#877d85]">{item.detail}</span>
            </span>
            <span class={`grid h-4 w-4 shrink-0 place-items-center rounded-[5px] border text-[0.6rem] font-black ${isSelected(item) ? "border-[#76517e] bg-[#76517e] text-white" : "border-[#ccc4cf] bg-white text-transparent"}`}>
              ✓
            </span>
          </button>
        ))}
      ...
      <div class="flex items-center justify-between border-t border-[#e7e2e8] bg-[#faf8fb] px-3 py-2">
        <span class="text-[0.58rem] font-medium text-[#8a8088]">{props.selectedItems.length} attached</span>
        <button
