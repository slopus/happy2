...
export function AgentMentionPicker(props: AgentMentionPickerProps) {
  const matchingAgents = () =>
    props.agents.filter((agent) => agent.name.toLowerCase().includes(props.query.toLowerCase()));
    ...
    >
      ...
      <div class="p-1.5">
        {matchingAgents().length === 0 && (
          <p class="px-3 py-5 text-center text-[0.68rem] text-[#8b8389]">No matching agents.</p>
        )}
          ...
          >
            ...
            <span class="min-w-0 flex-1">
              <span class="flex items-center gap-2">
                <span class="truncate text-[0.72rem] font-extrabold text-[#352d35]">{agent.name}</span>
                <span class={`h-1.5 w-1.5 rounded-full ${agent.status === "ready" ? "bg-[#45a968]" : "bg-[#5c83b6]"}`} />
              </span>
              <span class="mt-0.5 block truncate text-[0.61rem] text-[#81777f]">{agent.description}</span>
            </span>
            <span class="shrink-0 text-[0.57rem] font-bold capitalize text-[#918790]">{agent.status}</span>
          </button>
        ))}
      </div>
    </div>
