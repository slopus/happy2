  ...
  return (
    ...
    >
      <div class="border-b border-[#e7e2e8] bg-[#faf8fb] px-3 py-2">
        ...
        <p class="mt-0.5 text-[0.58rem] text-[#8d848c]">Choose what this agent may do for the request.</p>
      </div>
      ...
      <div class="p-1.5">
        {executionModes.map((option) => (
          ...
          >
            <span class={`mt-0.5 grid h-5 w-5 shrink-0 place-items-center rounded-full border text-[0.57rem] font-black ${props.selectedMode === option.id ? "border-[#76517e] bg-[#76517e] text-white" : "border-[#cfc7d2] bg-white text-transparent"}`}>
              ✓
            </span>
            <span class="min-w-0 flex-1">
              <span class="block text-[0.69rem] font-extrabold text-[#3b333b]">{option.label}</span>
              <span class="mt-0.5 block text-[0.58rem] leading-4 text-[#81777f]">{option.description}</span>
              <span class="mt-1 flex flex-wrap gap-1">
                {option.permissions.map((permission) => (
                  <span class="rounded bg-[#f2eef4] px-1.5 py-0.5 text-[0.52rem] font-bold text-[#776b79]">{permission}</span>
                ))}
              </span>
            </span>
          </button>
...
export function DelegationReceipt(props: { delegation: Delegation }) {
  return (
    ...
    >
      ...
      <span class="text-[0.62rem] font-extrabold text-[#443848]">{props.delegation.agentName}</span>
