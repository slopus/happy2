...
function renderMessageBody(body: string) {
  return body.split(/(@[A-Za-z][\w-]*)/g).map((part) =>
    part.startsWith("@") ? (
      <span class="rounded-[4px] bg-[#eee6f3] px-1 py-0.5 font-bold text-[#673b78]">{part}</span>
    ) : (
      part
    )
  );
    ...
    >
      <div class="flex min-h-full flex-col justify-end">
        <section class="px-6 pb-6 pt-12" aria-labelledby="conversation-intro-heading">
          ...
          <p class="mt-2 max-w-[720px] text-[0.82rem] leading-5 text-[#6f676d]">{props.description}</p>
        </section>
        ...
        {props.searchQuery && (
          <p class="mx-5 my-3 rounded-lg border border-[#ded5e7] bg-[#faf7fc] px-3 py-2 text-[0.72rem] font-medium text-[#66556f]" role="status">
            ...
            {(message) => (
              ...
              <div class="min-w-0 flex-1">
                <div class="flex items-baseline gap-2">
                  <h3 class="truncate text-[0.78rem] font-extrabold text-[#2e292d]">{message.author}</h3>
                  <time class="text-[0.61rem] font-medium text-[#9a9298]">{message.time}</time>
                </div>
