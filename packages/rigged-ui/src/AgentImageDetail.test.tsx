...
const BUILD_LOG = [
    "#3 [1/4] FROM docker.io/rigged/agent-base:latest",
...
it("holds AgentImageDetail layout, progress, and the Dockerfile + build log blocks", async () => {
    ...
    const badge = view.$('[data-testid="detail"] [data-rigged-ui="badge"]');
    ...
    const track = view.$(".rigged-agent-image-detail__progress-track");
    const fill = view.$(".rigged-agent-image-detail__progress-fill");
    ...
    expect(view.$(".rigged-agent-image-detail__progress-value").element.textContent).toBe("62%");

    // Dockerfile block: exact source, monospace, on the code surface (#141319).
    const dockerfile = view.$('[data-rigged-ui="agent-image-detail-dockerfile"]');
    ...
    const log = view.$('[data-rigged-ui="agent-image-detail-log"]');
    expect(log.element.textContent).toBe(BUILD_LOG);
    expect(log.computedStyle("font-family")).toBe(monoFamily());
    const notes = view
        .$('[data-testid="detail"]')
        .element.querySelectorAll(".rigged-agent-image-detail__section-note");
...
it("handles failed, empty-log, loading, and error states", async () => {
    ...
    expect(
        view
            .$('[data-testid="failed"] [data-rigged-ui="badge"]')
            .element.getAttribute("data-variant"),
    ).toBe("danger");
    const failBanner = view.$('[data-testid="failed"] [data-rigged-ui="banner"]');
    ...
    expect(
        view
            .$('[data-testid="empty"]')
            .element.querySelector('[data-rigged-ui="agent-image-detail-log"]'),
        "no log block when empty",
    ).toBeNull();
    expect(
        view.$('[data-testid="empty"] .rigged-agent-image-detail__empty').element.textContent,
    ).toBe("No build output yet.");

    // Loading: both blocks are placeholders, none rendered yet.
    const loading = view.$('[data-testid="loading"]');
    expect(
        loading.element.querySelector('[data-rigged-ui="agent-image-detail-dockerfile"]'),
    ).toBeNull();
    expect(loading.element.querySelector('[data-rigged-ui="agent-image-detail-log"]')).toBeNull();
    expect(
        Array.from(
            loading.element.querySelectorAll(".rigged-agent-image-detail__empty"),
            (n) => n.textContent,
        ),
    ...
    expect(
        errorPanel.element.querySelector('[data-rigged-ui="agent-image-detail-dockerfile"]'),
    ).toBeNull();
