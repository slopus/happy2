...
export function Banner(props: BannerProps) {
    ...
    return (
        <div
            class={["rigged-banner", local.class].filter(Boolean).join(" ")}
            data-rigged-ui="banner"
        ...
        >
            <Show when={local.icon}>
                {(name) => (
                    <span class="rigged-banner__icon" data-rigged-ui="banner-icon">
                        <Icon name={name()} size={16} />
                    </span>
                )}
            </Show>
            <div class="rigged-banner__content" data-rigged-ui="banner-content">
                <Show when={local.title}>
                    <span class="rigged-banner__title" data-rigged-ui="banner-title">
                        {local.title}
                    </span>
                </Show>
                <span class="rigged-banner__message" data-rigged-ui="banner-message">
                    {local.children}
                </span>
            </div>
            <Show when={hasActions()}>
                <div class="rigged-banner__actions" data-rigged-ui="banner-actions">
                    <Show when={local.action}>
                        {(action) => (
                            <Button
                                class="rigged-banner__action"
                    ...
                    <Show when={local.onDismiss}>
                        <button
                            aria-label="Dismiss"
                            class="rigged-banner__dismiss"
                            data-rigged-ui="banner-dismiss"
