...
const FIGTREE = "Rigged Figtree, system-ui, sans-serif";
...
it("holds SearchResults geometry, group headers, row layouts, highlight, and optical alignment", async () => {
    ...
    for (const spec of groupHeads) {
        const section = q(`[data-rigged-ui="search-results-group"][data-type="${spec.type}"]`);
        const head = view.$(
            `[data-testid="results"] [data-type="${spec.type}"] [data-rigged-ui="search-results-group-head"]`,
        );
        ...
        const label = view.$(
            `[data-testid="results"] [data-type="${spec.type}"] [data-rigged-ui="search-results-group-label"]`,
        );
        ...
        const count = view.$(
            `[data-testid="results"] [data-type="${spec.type}"] [data-rigged-ui="search-results-group-count"]`,
        );
    ...
    for (const [id, y] of Object.entries(rowY)) {
        ...
        const leading = q(`[data-item-id="${id}"] [data-rigged-ui="search-results-row-leading"]`);
        ...
        const body = q(`[data-item-id="${id}"] [data-rigged-ui="search-results-row-body"]`);
    ...
    for (const id of ["launch-week", "m2"] as const) {
        const glyph = q(`[data-item-id="${id}"] [data-rigged-ui="search-results-row-glyph"]`);
        ...
        const icon = q(`[data-item-id="${id}"] [data-rigged-ui="search-results-row-glyph"] svg`);
    ...
    for (const id of ["maya", "jun", "m1"] as const) {
        const avatar = q(`[data-item-id="${id}"] [data-rigged-ui="avatar"]`);
    ...
    expect(q('[data-item-id="maya"] [data-rigged-ui="avatar-initials"]').element.textContent).toBe(
        "MJ",
    );
    ...
    for (const id of ["maya", "jun", "m1", "m2"] as const) {
        const title = q(`[data-item-id="${id}"] [data-rigged-ui="search-results-row-title"]`);
        ...
        const meta = q(`[data-item-id="${id}"] [data-rigged-ui="search-results-row-meta"]`);
    ...
    const marks = view.container.querySelectorAll(
        '[data-testid="results"] [data-item-id="launch-week"] [data-rigged-ui="search-results-mark"]',
    );
    expect(marks.length, "launch-week mark count").toBe(1);
    const mark = q('[data-item-id="launch-week"] [data-rigged-ui="search-results-mark"]');
    ...
    await paints(q('[data-item-id="m1"] [data-rigged-ui="search-results-mark"]'), "m1 mark");
    /* A non-matching title carries no mark. */
    expect(
        view.container.querySelectorAll(
            '[data-testid="results"] [data-item-id="maya"] [data-rigged-ui="search-results-mark"]',
        ).length,
        "maya has no mark",
    ).toBe(0);
...
it("renders the empty state and rich message snippets", async () => {
    ...
    const emptyLabel = view.$(
        '[data-testid="empty"] [data-rigged-ui="search-results-empty-label"]',
    );
    ...
    const emptyIcon = view.$(
        '[data-testid="empty"] [data-rigged-ui="search-results-empty-icon"] svg',
    );
    expect(emptyIcon.bounds().width).toBe(20);
    await paints(emptyIcon, "empty icon");
    /* No rows exist in the empty state. */
    expect(
        view.container.querySelector('[data-testid="empty"] [data-rigged-ui="search-results-row"]'),
    ).toBeNull();
    ...
    const code = view.$('[data-testid="rich"] [data-rigged-ui="search-results-code"]');
    expect(code.element.textContent).toBe("deploy");
    expect(code.textMetrics().font.family).toBe(MONO);
    await paints(code, "rich code");

    const link = view.$('[data-testid="rich"] [data-rigged-ui="search-results-link"]');
    ...
    const richMark = view.$('[data-testid="rich"] [data-rigged-ui="search-results-mark"]');
