...
const DOT_COLORS: Record<Availability, string> = {
    automatic: "rgb(117, 112, 133)", // --rg-text-muted #757085
    online: "rgb(52, 211, 153)", // --rg-success  #34d399
    away: "rgb(251, 191, 36)", // --rg-warning  #fbbf24
    dnd: "rgb(248, 113, 113)", // --rg-danger   #f87171
};
...
it("holds StatusPicker card, availability dots, segmented layout, and status field", async () => {
    ...
    // ---- Availability track --------------------------------------------
    const track = view.$(sp(' [data-rigged-ui="status-picker-segmented"]'));
    ...
    expect(track.computedStyle("--rigged-sp-index"), "pill index").toBe("1");
    expect(
        view.$(sp()).element.querySelectorAll('[data-rigged-ui="status-picker-segment"]').length,
        "segment count",
    ).toBe(4);
    ...
    for (const value of AVAILABILITY) {
        const seg = view.$(
            sp(` [data-availability="${value}"][data-rigged-ui="status-picker-segment"]`),
        );
        ...
        const dot = view.$(
            sp(` [data-availability="${value}"][data-rigged-ui="status-picker-dot"]`),
        );
        ...
        const label = view.$(
            sp(` [data-availability="${value}"] [data-rigged-ui="status-picker-segment-label"]`),
        );
    ...
    const onlineLabel = view.$(
        sp(' [data-availability="online"] [data-rigged-ui="status-picker-segment-label"]'),
    );
    ...
    const pill = view.$(sp(' [data-rigged-ui="status-picker-pill"]'));
    ...
    const availLabel = view.$(sp(' [data-rigged-ui="status-picker-availability-label"]'));
    expect(
    ...
    // ---- Status field ---------------------------------------------------
    const field = view.$(sp(' [data-rigged-ui="status-picker-field"]'));
    ...
    const emoji = view.$(sp(' [data-rigged-ui="status-picker-emoji"]'));
    ...
    const input = view.$(sp(' [data-rigged-ui="status-picker-input"]'));
    ...
    const clear = view.$(sp(' [data-rigged-ui="status-picker-field"] [data-rigged-ui="button"]'));
    expect(clear.bounds(), "clear button box").toMatchObject({ width: 28, height: 28 });
    const clearGlyph = view.$(
        sp(' [data-rigged-ui="status-picker-field"] [data-rigged-ui="button"] svg'),
    );
    ...
    // ---- Expiry meta ----------------------------------------------------
    const meta = view.$(sp(' [data-rigged-ui="status-picker-meta"]'));
    expect(meta.computedStyle("color"), "meta color").toBe("rgb(117, 112, 133)");
    expect(
        view.$(sp(' [data-rigged-ui="status-picker-meta-icon"] [data-rigged-ui="icon"]')).bounds(),
        "meta icon box",
    ).toMatchObject({ width: 14, height: 14 });

    // ---- Layout rhythm (8px within section, 16px between sections) ------
    const availSection = view.$(sp(' [data-rigged-ui="status-picker-availability"]'));
    const statusSection = view.$(sp(' [data-rigged-ui="status-picker-status"]'));
    ...
    const statusLabelBounds = view.$(sp(' [data-rigged-ui="status-picker-status-label"]')).bounds();
...
it("holds StatusPicker availability sweep and empty-status state", async () => {
    ...
    for (const [index, value] of AVAILABILITY.entries()) {
        ...
        const track = view.$(`${root} [data-rigged-ui="status-picker-segmented"]`);
        expect(track.computedStyle("--rigged-sp-index"), `${value} index`).toBe(String(index));

        const seg = view.$(
            `${root} [data-availability="${value}"][data-rigged-ui="status-picker-segment"]`,
        );
        const pill = view.$(`${root} [data-rigged-ui="status-picker-pill"]`);
        ...
        const selectedLabel = view.$(
            `${root} [data-availability="${value}"] [data-rigged-ui="status-picker-segment-label"]`,
        );
        ...
        for (const other of AVAILABILITY) {
            ...
            const otherLabel = view.$(
                `${root} [data-availability="${other}"] [data-rigged-ui="status-picker-segment-label"]`,
            );
            ...
            const otherDot = view.$(
                `${root} [data-availability="${other}"][data-rigged-ui="status-picker-dot"]`,
            );
    ...
    const placeholderGlyph = view.$(
        '[data-testid="empty"] [data-rigged-ui="status-picker-emoji"] [data-rigged-ui="icon"]',
    );
    ...
    expect(
        emptyRoot.element.querySelector(
            '[data-rigged-ui="status-picker-field"] [data-rigged-ui="button"]',
        ),
        "no clear button when empty",
    ).toBe(null);
    expect(
        emptyRoot.element.querySelector('[data-rigged-ui="status-picker-meta"]'),
        "no meta when no expiry",
    ).toBe(null);
    const emptyInput = view.$('[data-testid="empty"] [data-rigged-ui="status-picker-input"]');
