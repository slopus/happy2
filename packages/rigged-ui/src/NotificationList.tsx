...
function renderSegment(segment: MessageSegment): JSX.Element {
    switch (segment.kind) {
        case "mention":
            return (
                <span
                    class="rigged-notification-row__mention"
                    data-rigged-ui="notification-mention"
                >
                    @{segment.text}
                </span>
            );
        case "code":
            return (
                <code class="rigged-notification-row__code" data-rigged-ui="notification-code">
                    {segment.text}
                </code>
            );
        case "link":
            return (
                <a class="rigged-notification-row__link" data-rigged-ui="notification-link">
                    {segment.text}
                </a>
            );
        default:
...
function NotificationRow(props: { item: NotificationItem; onSelect?: (id: string) => void }) {
    ...
    return (
        <button
            class="rigged-notification-row"
            data-item-id={item().id}
            data-kind={item().kind}
            data-rigged-ui="notification-row"
        ...
        >
            <span
                aria-hidden="true"
                class="rigged-notification-row__unread-lane"
                data-rigged-ui="notification-unread-lane"
            >
                <Show when={item().unread}>
                    <span
                        class="rigged-notification-row__unread"
                        data-rigged-ui="notification-unread"
                    />
                </Show>
            </span>
            <span class="rigged-notification-row__media" data-rigged-ui="notification-media">
                <Show
                    when={item().actor}
                    fallback={
                        <span
                            aria-label={meta().label}
                            class="rigged-notification-row__kind"
                            data-rigged-ui="notification-kind"
                            data-tone={meta().tone}
                            data-variant="tile"
                            role="img"
                        >
            ...
            <span class="rigged-notification-row__body" data-rigged-ui="notification-body">
                <span class="rigged-notification-row__text" data-rigged-ui="notification-text">
                    <Show when={item().actor}>
                        {(actor) => (
                            <>
                                <span
                                    class="rigged-notification-row__actor"
                                    data-rigged-ui="notification-actor"
                                >
                                    {actor().name}
                                </span>{" "}
                            </>
                ...
                <Show when={item().context}>
                    <span
                        class="rigged-notification-row__context"
                        data-rigged-ui="notification-context"
                    >
                        {item().context}
                    </span>
                </Show>
            </span>
            <span class="rigged-notification-row__time" data-rigged-ui="notification-time">
                {item().time}
            </span>
        </button>
    );
...
export function NotificationList(props: NotificationListProps) {
    ...
    return (
        <div
            {...rest}
            class={["rigged-notification-list", local.class].filter(Boolean).join(" ")}
            data-rigged-ui="notification-list"
            style={local.style}
        >
            <Show
                ...
                fallback={
                    <div
                        class="rigged-notification-list__empty"
                        data-rigged-ui="notification-list-empty"
                    >
