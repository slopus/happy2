...
it("holds pending ApprovalCard geometry, typography, and interactions", async () => {
    ...
    const chip = view.$('[data-testid="ac-pending"] [data-rigged-ui="approval-card-chip"]');
    ...
    const shield = view.$(
        '[data-testid="ac-pending"] [data-rigged-ui="approval-card-chip"] [data-rigged-ui="icon"]',
    );
    ...
    const badge = view.$('[data-testid="ac-pending"] [data-rigged-ui="badge"]');
    ...
    const avatar = view.$('[data-testid="ac-pending"] [data-rigged-ui="avatar"]');
    ...
    const header = view.$('[data-testid="ac-pending"] [data-rigged-ui="approval-card-header"]');
    const agentName = view.$(
        '[data-testid="ac-pending"] [data-rigged-ui="approval-card-agent-name"]',
    );
    ...
    const agent = view.$('[data-testid="ac-pending"] [data-rigged-ui="approval-card-agent"]');
    expect(card.bounds().x + 680 - (agent.bounds().x + agent.bounds().width)).toBe(17);

    /* ---- Title / reason / action well ---------------------------------- */

    const title = view.$('[data-testid="ac-pending"] [data-rigged-ui="approval-card-title"]');
    ...
    const reason = view.$('[data-testid="ac-pending"] [data-rigged-ui="approval-card-reason"]');
    ...
    const action = view.$('[data-testid="ac-pending"] [data-rigged-ui="approval-card-action"]');
    ...
    expect(action.textMetrics().font.family).toBe("Rigged Mono, ui-monospace, monospace");
    ...
    const actionText = view.$(
        '[data-testid="ac-pending"] [data-rigged-ui="approval-card-action-text"]',
    );
    ...
    expect(
        view.container.querySelector(
            '[data-testid="ac-pending"] [data-rigged-ui="approval-card-details"]',
        ),
    ).toBeNull();

    /* ---- Footer actions -------------------------------------------------- */

    const footer = view.$('[data-testid="ac-pending"] .rigged-approval-card__footer');
    ...
    const toggle = view.$('[data-testid="ac-pending"] [data-rigged-ui="approval-card-toggle"]');
    ...
    const toggleLabel = view.$(
        '[data-testid="ac-pending"] [data-rigged-ui="approval-card-toggle-label"]',
    );
    ...
    const chevron = view.$(
        '[data-testid="ac-pending"] [data-rigged-ui="approval-card-toggle"] [data-rigged-ui="icon"]',
    );
    ...
    const details = view.$('[data-testid="ac-expanded"] [data-rigged-ui="approval-card-details"]');
    expect(details.bounds().y - expanded.bounds().y).toBe(145);

    const labels = Array.from(
        view.container.querySelectorAll(
            '[data-testid="ac-expanded"] [data-rigged-ui="approval-card-detail-label"]',
        ),
    );
    ...
    expect(impactLabelMetrics.font.family).toBe("Rigged Mono, ui-monospace, monospace");
    ...
    const impact = view.$('[data-testid="ac-expanded"] [data-rigged-ui="approval-card-impact"]');
    ...
    const resources = Array.from(
        view.container.querySelectorAll(
            '[data-testid="ac-expanded"] [data-rigged-ui="approval-card-resources"] [data-rigged-ui="badge"]',
        ),
    );
    ...
    const upChevron = view.$(
        '[data-testid="ac-expanded"] [data-rigged-ui="approval-card-toggle"] [data-rigged-ui="icon"]',
    );
    ...
    expect(
        view
            .$('[data-testid="ac-expanded"] [data-rigged-ui="approval-card-toggle"]')
...
it("holds resolved ApprovalCard banners, state lines, and optical centering", async () => {
    ...
    for (const id of ["ac-approved", "ac-denied"] as const) {
        ...
        const banner = view.$(`[data-testid="${id}"] [data-rigged-ui="approval-card-banner"]`);
        ...
        const bannerLabel = view.$(
            `[data-testid="${id}"] [data-rigged-ui="approval-card-banner-label"]`,
        );
        const bannerMetrics = bannerLabel.textMetrics();
        expect(bannerMetrics.text).toBe(id === "ac-approved" ? "Approved" : "Denied");
        expect(bannerMetrics.font.family).toBe("Rigged Mono, ui-monospace, monospace");
        ...
        const bannerIcon = view.$(
            `[data-testid="${id}"] [data-rigged-ui="approval-card-banner"] [data-rigged-ui="icon"]`,
        );
        ...
        expect(
            view.container.querySelector(`[data-testid="${id}"] [data-rigged-ui="button"]`),
        ).toBeNull();
        const state = view.$(`[data-testid="${id}"] [data-rigged-ui="approval-card-state"]`);
        expect(state.computedStyle("color")).toBe("rgb(117, 112, 133)");
        const stateLabel = view.$(
            `[data-testid="${id}"] [data-rigged-ui="approval-card-state-label"]`,
        );
        ...
        const stateIcon = view.$(
            `[data-testid="${id}"] [data-rigged-ui="approval-card-state"] [data-rigged-ui="icon"]`,
        );
        ...
        expect(
            view
                .$(`[data-testid="${id}"] [data-rigged-ui="approval-card-toggle"]`)
                .element.getAttribute("aria-expanded"),
        ).toBe("false");
    }

    /* Resolution-specific colors. */
    const banner = view.$('[data-testid="ac-approved"] [data-rigged-ui="approval-card-banner"]');
    ...
    expect(
        view
            .$('[data-testid="ac-approved"] [data-rigged-ui="approval-card-chip"]')
    ...
    expect(
        view
            .$('[data-testid="ac-approved"] [data-rigged-ui="badge"]')
            .element.getAttribute("data-variant"),
    ).toBe("neutral");
    expect(
        view
            .$(
                '[data-testid="ac-approved"] [data-rigged-ui="approval-card-state"] [data-rigged-ui="icon"]',
            )
            .computedStyle("color"),
    ).toBe("rgb(52, 211, 153)");
    const deniedBanner = view.$(
        '[data-testid="ac-denied"] [data-rigged-ui="approval-card-banner"]',
    );
    ...
    expect(
        view
            .$('[data-testid="ac-denied"] [data-rigged-ui="approval-card-chip"]')
            .computedStyle("color"),
...
it("holds expanded resolved ApprovalCards and fluid widths", async () => {
    ...
    for (const id of ["ac-approved-x", "ac-denied-x"] as const) {
        ...
        const details = view.$(`[data-testid="${id}"] [data-rigged-ui="approval-card-details"]`);
        expect(details.bounds().y - cardEl.bounds().y).toBe(177); /* banner 32 + body 145 */
        const resources = Array.from(
            view.container.querySelectorAll(
                `[data-testid="${id}"] [data-rigged-ui="approval-card-resources"] [data-rigged-ui="badge"]`,
            ),
        );
        expect(resources.length).toBe(3);
        ...
        const chip = view.$(`[data-testid="${id}"] [data-rigged-ui="approval-card-chip"]`);
        const shield = view.$(
            `[data-testid="${id}"] [data-rigged-ui="approval-card-chip"] [data-rigged-ui="icon"]`,
        );
    ...
    const narrowAction = view.$(
        '[data-testid="ac-narrow"] [data-rigged-ui="approval-card-action"]',
    );
    ...
    const narrowActionText = view.$(
        '[data-testid="ac-narrow"] [data-rigged-ui="approval-card-action-text"]',
    );
    ...
    expect(view.$('[data-testid="ac-narrow"] [data-rigged-ui="badge"]').textMetrics().text).toBe(
        "ESCALATION",
    );
