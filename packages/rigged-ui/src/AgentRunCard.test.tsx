...
it("holds the review hero card geometry, mint treatment, and interactions at 680", async () => {
    ...
    const header = view.$('[data-testid="run-review"] [data-rigged-ui="agent-run-card-header"]');
    expect(header.bounds()).toEqual({ x: 29, y: 29, width: 646, height: 28 });
    const avatar = view.$('[data-testid="run-review"] [data-rigged-ui="avatar"]');
    expect(avatar.bounds()).toEqual({ x: 29, y: 29, width: 28, height: 28 });
    expect(avatar.computedStyle("border-radius")).toBe("7px");

    const agent = view.$('[data-testid="run-review"] [data-rigged-ui="agent-run-card-agent"]');
    ...
    const kind = view.$('[data-testid="run-review"] [data-rigged-ui="agent-run-card-kind"]');
    ...
    const badge = view.$('[data-testid="run-review"] [data-rigged-ui="badge"]');
    ...
    const toggle = view.$('[data-testid="run-review"] [data-rigged-ui="agent-run-card-toggle"]');
    ...
    const title = view.$('[data-testid="run-review"] [data-rigged-ui="agent-run-card-title"]');
    ...
    const meta = view.$('[data-testid="run-review"] [data-rigged-ui="agent-run-card-meta"]');
    expect(meta.bounds()).toEqual({ x: 29, y: 93, width: 646, height: 16 });
    const added = view.$('[data-testid="run-review"] [data-rigged-ui="agent-run-card-added"]');
    const removed = view.$('[data-testid="run-review"] [data-rigged-ui="agent-run-card-removed"]');
    const detail = view.$('[data-testid="run-review"] [data-rigged-ui="agent-run-card-detail"]');
    ...
    /* — branch row — */
    const branch = view.$('[data-testid="run-review"] [data-rigged-ui="agent-run-card-branch"]');
    expect(branch.bounds()).toEqual({ x: 29, y: 117, width: 646, height: 16 });
    const branchName = view.$(
        '[data-testid="run-review"] [data-rigged-ui="agent-run-card-branch-name"]',
    );
    ...
    const branchIcon = view.$(
        '[data-testid="run-review"] [data-rigged-ui="agent-run-card-branch-icon"] [data-rigged-ui="icon"]',
    );
    ...
    const actions = view.$('[data-testid="run-review"] [data-rigged-ui="agent-run-card-actions"]');
    expect(actions.bounds()).toEqual({ x: 29, y: 145, width: 646, height: 28 });
    expect(actions.element.querySelectorAll('[data-rigged-ui="button"]').length).toBe(2);
    const primary = view.$(
        '[data-testid="run-review"] [data-rigged-ui="button"][data-variant="primary"]',
    );
    ...
    const secondary = view.$(
        '[data-testid="run-review"] [data-rigged-ui="button"][data-variant="secondary"]',
    );
    ...
    expect(review.element.querySelector('[data-rigged-ui="agent-run-card-steps"]')).toBeNull();
    expect(review.element.querySelector('[data-rigged-ui="agent-run-card-body"]')).toBeNull();
...
it("holds the working and queued status treatments at 680 and 440", async () => {
    ...
    const progress = view.$(
        '[data-testid="run-working"] [data-rigged-ui="agent-run-card-progress"]',
    );
    ...
    const fill = view.$(
        '[data-testid="run-working"] [data-rigged-ui="agent-run-card-progress-fill"]',
    );
    ...
    const workingBadge = view.$('[data-testid="run-working"] [data-rigged-ui="badge"]');
    ...
    const queuedBadge = view.$('[data-testid="run-queued"] [data-rigged-ui="badge"]');
    ...
    expect(queued.element.querySelector('[data-rigged-ui="agent-run-card-progress"]')).toBeNull();
    ...
    const completeBadge = view.$('[data-testid="run-complete"] [data-rigged-ui="badge"]');
    expect(completeBadge.element.textContent).toBe("COMPLETED");
    expect(completeBadge.offsets().top).toBe(5);
    const check = view.$('[data-testid="run-complete"] [data-rigged-ui="agent-run-card-check"]');
    expect(check.computedStyle("color")).toBe("rgb(52, 211, 153)");
    expect(check.element.querySelector('[data-rigged-ui="icon"]')?.getAttribute("data-name")).toBe(
        "check-circle",
    );
    expect(complete.element.querySelector('[data-rigged-ui="agent-run-card-steps"]')).toBeNull();
...
it("holds the expanded complete card at 440: steps, children slot, and max-width", async () => {
    ...
    const check = view.$('[data-testid="run-complete"] [data-rigged-ui="agent-run-card-check"]');
    expect(check.computedStyle("color")).toBe("rgb(52, 211, 153)");
    const badge = view.$('[data-testid="run-complete"] [data-rigged-ui="badge"]');
    expect(badge.element.textContent).toBe("COMPLETED");
    const toggle = view.$('[data-testid="run-complete"] [data-rigged-ui="agent-run-card-toggle"]');
    expect(toggle.element.getAttribute("aria-expanded")).toBe("true");
    const toggleIcon = view.$(
        '[data-testid="run-complete"] [data-rigged-ui="agent-run-card-toggle-icon"]',
    );
    expect(toggleIcon.computedStyle("transform")).toBe("matrix(-1, 0, 0, -1, 0, 0)");

    /* — 4-digit / 1-digit diffstat variant — */
    const added = view.$('[data-testid="run-complete"] [data-rigged-ui="agent-run-card-added"]');
    const removed = view.$(
        '[data-testid="run-complete"] [data-rigged-ui="agent-run-card-removed"]',
    );
    const detail = view.$('[data-testid="run-complete"] [data-rigged-ui="agent-run-card-detail"]');
    ...
    const meta = view.$('[data-testid="run-complete"] [data-rigged-ui="agent-run-card-meta"]');
    expect(meta.bounds()).toEqual({ x: 29, y: 93, width: 406, height: 16 });
    const branch = view.$('[data-testid="run-complete"] [data-rigged-ui="agent-run-card-branch"]');
    ...
    const stepsList = view.$(
        '[data-testid="run-complete"] [data-rigged-ui="agent-run-card-steps"]',
    );
    ...
    const doneGlyph = view.$(
        '[data-testid="run-complete"] [data-status="done"] [data-rigged-ui="agent-run-card-step-glyph"]',
    );
    ...
    const workingDot = view.$(
        '[data-testid="run-complete"] li[data-status="working"] [data-rigged-ui="agent-run-card-step-dot"]',
    );
    ...
    const pendingDot = view.$(
        '[data-testid="run-complete"] [data-status="pending"] [data-rigged-ui="agent-run-card-step-dot"]',
    );
    expect(
    ...
    const doneLabel = view.$(
        '[data-testid="run-complete"] [data-status="done"] [data-rigged-ui="agent-run-card-step-label"]',
    );
    const workingLabel = view.$(
        '[data-testid="run-complete"] li[data-status="working"] [data-rigged-ui="agent-run-card-step-label"]',
    );
    const pendingLabel = view.$(
        '[data-testid="run-complete"] [data-status="pending"] [data-rigged-ui="agent-run-card-step-label"]',
    );
    ...
    const body = view.$('[data-testid="run-complete"] [data-rigged-ui="agent-run-card-body"]');
    expect(body.bounds()).toEqual({ x: 29, y: 237, width: 406, height: 40 });
    const actions = view.$(
        '[data-testid="run-complete"] [data-rigged-ui="agent-run-card-actions"]',
    );
...
it("holds every status expanded, including narrow 440 title wrap", async () => {
    ...
    expect(review.element.querySelectorAll('[data-rigged-ui="agent-run-card-step"]').length).toBe(
        3,
    );
    expect(review.element.querySelector('[data-rigged-ui="agent-run-card-body"]')).not.toBeNull();

    /* — working expanded at 440: long title wraps to exactly two lines — */
    const workingTitle = view.$(
        '[data-testid="run-working"] [data-rigged-ui="agent-run-card-title"]',
    );
    ...
    expect(
        workingCard.element.querySelector('[data-rigged-ui="agent-run-card-progress"]'),
    ).not.toBeNull();

    /* — queued expanded with zero steps renders no checklist — */
    const queued = view.$('[data-testid="run-queued"]');
    expect(queued.bounds()).toEqual({ x: 12, y: 12, width: 440, height: 90 });
    expect(queued.element.querySelector('[data-rigged-ui="agent-run-card-steps"]')).toBeNull();
...
it("centers ink optically in every row, glyph, and counter", async () => {
    ...
    const header = rv('[data-rigged-ui="agent-run-card-header"]');
    const headerCy = boxCenterY(header);
    const agentInk = await inkCenter(rv('[data-rigged-ui="agent-run-card-agent"]'));
    ...
    const kindInk = await inkCenter(rv('[data-rigged-ui="agent-run-card-kind"]'));
     ...
     * loose. */
    const avatar = rv('[data-rigged-ui="avatar"]');
    expect(avatar.bounds().y).toBe(29);
    const initialsInk = await inkCenter(rv('[data-rigged-ui="avatar-initials"]'));
    ...
    for (const [card, label] of [
        ...
        const badge = view.$(`[data-testid="${card}"] [data-rigged-ui="badge"]`);
        expect(badge.element.textContent).toBe(label);
        expect(badge.offsets().top).toBe(5);
        const cardHeaderCy = boxCenterY(
            view.$(`[data-testid="${card}"] [data-rigged-ui="agent-run-card-header"]`),
        );
        const badgeInk = await inkCenter(
            view.$(`[data-testid="${card}"] [data-rigged-ui="badge-label"]`),
        );
    ...
    const toggleExpanded = rv('[data-rigged-ui="agent-run-card-toggle"]');
    ...
    const toggleCollapsed = view.$(
        '[data-testid="run-complete"] [data-rigged-ui="agent-run-card-toggle"]',
    );
    ...
    const titleInk = await inkCenter(rv('[data-rigged-ui="agent-run-card-title"]'));
    ...
    const metaCy = boxCenterY(rv('[data-rigged-ui="agent-run-card-meta"]'));
    const addedInk = await inkCenter(rv('[data-rigged-ui="agent-run-card-added"]'));
    expect(Math.abs(addedInk.y - metaCy)).toBeLessThanOrEqual(0.4);
    const removedInk = await inkCenter(rv('[data-rigged-ui="agent-run-card-removed"]'));
    ...
    const detailInk = await inkCenter(rv('[data-rigged-ui="agent-run-card-detail"]'));
    expect(Math.abs(detailInk.y - metaCy)).toBeLessThanOrEqual(0.75);

    /* — branch row: icon on both axes, mono name on the row center — */
    const branchCy = boxCenterY(rv('[data-rigged-ui="agent-run-card-branch"]'));
    const branchIcon = rv('[data-rigged-ui="agent-run-card-branch-icon"]');
    ...
    const branchNameInk = await inkCenter(rv('[data-rigged-ui="agent-run-card-branch-name"]'));
    ...
    for (const status of ["done", "working", "pending"] as const) {
        ...
        const glyph = rv(
            `li[data-status="${status}"] [data-rigged-ui="agent-run-card-step-glyph"]`,
        );
        ...
        const labelInk = await inkCenter(
            rv(`li[data-status="${status}"] [data-rigged-ui="agent-run-card-step-label"]`),
        );
    ...
    const primary = rv('[data-rigged-ui="button"][data-variant="primary"]');
    const secondary = rv('[data-rigged-ui="button"][data-variant="secondary"]');
    expect(primary.bounds().height).toBe(28);
    ...
    const primaryLabelInk = await inkCenter(
        rv('[data-rigged-ui="button"][data-variant="primary"] [data-rigged-ui="button-label"]'),
    );
