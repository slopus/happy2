...
const MONO = "Rigged Mono, ui-monospace, monospace";
...
async function emojiInk(view: Renderer, cellSelector: string, inner: "glyph" | "image") {
    const art = view.$(`${cellSelector} [data-rigged-ui="emoji-picker-art"]`);
    const el = view.$(`${cellSelector} [data-rigged-ui="emoji-picker-${inner}"]`);
...
it("holds EmojiPicker card, search, fixed grid slots, and per-emoji centering", async () => {
    ...
    const control = view.$('[data-testid="picker"] [data-rigged-ui="text-field-control"]');
    ...
    const input = view.$('[data-testid="picker"] [data-rigged-ui="text-field-input"]')
        .element as HTMLInputElement;
    expect(input.type).toBe("search");
    expect(input.value).toBe("");
    expect(input.placeholder).toBe("Search emoji");
    const searchIcon = view.$('[data-testid="picker"] [data-rigged-ui="text-field-icon"] svg');
    ...
    const grid = view.$('[data-testid="picker"] [data-rigged-ui="emoji-picker-grid"]');
    ...
    emoji.forEach((item, index) => {
        const cell = view.$(
            `[data-testid="picker"] [data-rigged-ui="emoji-picker-grid"] [data-emoji-id="${item.id}"]`,
        );
        const b = cell.bounds();
    ...
    for (const item of emoji) {
        const cell = `[data-testid="picker"] [data-rigged-ui="emoji-picker-grid"] [data-emoji-id="${item.id}"]`;
        const art = view.$(`${cell} [data-rigged-ui="emoji-picker-art"]`);
    ...
    for (const id of ["relay", "mint"]) {
        const image = view.$(
            `[data-testid="picker"] [data-rigged-ui="emoji-picker-grid"] [data-emoji-id="${id}"] [data-rigged-ui="emoji-picker-image"]`,
        );
    ...
    const probeGlyph = view.$(
        '[data-testid="picker"] [data-rigged-ui="emoji-picker-grid"] [data-emoji-id="thumbsup"] [data-rigged-ui="emoji-picker-glyph"]',
    );
    expect(probeGlyph.computedStyle("--rg-emoji-x"), "emoji-x correction").toBe(
        emojiCorrection[engine].x,
    );
    expect(probeGlyph.computedStyle("--rg-emoji-y"), "emoji-y correction").toBe(
        emojiCorrection[engine].y,
    );

    for (const id of inkSubset) {
        const item = emoji.find((entry) => entry.id === id)!;
        const cell = `[data-testid="picker"] [data-rigged-ui="emoji-picker-grid"] [data-emoji-id="${id}"]`;
    ...
    (
        view.$(
            '[data-testid="picker"] [data-rigged-ui="emoji-picker-grid"] [data-emoji-id="thumbsup"]',
        ).element as HTMLButtonElement
    ).click();
    (
        view.$(
            '[data-testid="picker"] [data-rigged-ui="emoji-picker-grid"] [data-emoji-id="relay"]',
        ).element as HTMLButtonElement
    ).click();
...
it("holds EmojiPicker recent sections, searching, empty state, and query events", async () => {
    ...
    const recentLabel = view.$(
        '[data-testid="recent"] [data-rigged-ui="emoji-picker-recent-label"]',
    );
    ...
    const allLabel = view.$('[data-testid="recent"] [data-rigged-ui="emoji-picker-all-label"]');
    expect(allLabel.element.textContent).toBe("All emoji");

    const recentGrid = view.$('[data-testid="recent"] [data-rigged-ui="emoji-picker-recent-grid"]');
    const recentCells = recentGrid.element.querySelectorAll('[data-rigged-ui="emoji-picker-cell"]');
    ...
    ["thumbsup", "tada", "rocket", "fire", "heart", "relay"].forEach((id, index) => {
        const cell = view.$(
            `[data-testid="recent"] [data-rigged-ui="emoji-picker-recent-grid"] [data-emoji-id="${id}"]`,
        );
    ...
    const allGrid = view.$('[data-testid="recent"] [data-rigged-ui="emoji-picker-grid"]');
    expect(
        allGrid.element.querySelectorAll('[data-rigged-ui="emoji-picker-cell"]').length,
        "all cell count",
    ).toBe(24);
    ...
    (
        view.$(
            '[data-testid="recent"] [data-rigged-ui="emoji-picker-recent-grid"] [data-emoji-id="fire"]',
        ).element as HTMLButtonElement
    ).click();
    ...
    expect(
        searching.element.querySelector('[data-rigged-ui="emoji-picker-recent-section"]'),
        "recent hidden while searching",
    ).toBeNull();
    expect(
        searching.element.querySelector('[data-rigged-ui="emoji-picker-all-label"]'),
        "no all-label without a recent row",
    ).toBeNull();
    const searchInput = view.$('[data-testid="searching"] [data-rigged-ui="text-field-input"]')
        .element as HTMLInputElement;
    expect(searchInput.value, "query drives the field").toBe("fire");
    expect(
        view
            .$('[data-testid="searching"] [data-rigged-ui="emoji-picker-grid"]')
            .element.querySelectorAll('[data-rigged-ui="emoji-picker-cell"]').length,
    ...
    expect(
        empty.element.querySelector('[data-rigged-ui="emoji-picker-grid"]'),
        "no grid when empty",
    ).toBeNull();
    const emptyMsg = view.$('[data-testid="empty"] [data-rigged-ui="emoji-picker-empty"]');
