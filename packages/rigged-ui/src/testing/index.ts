...
function identifier(element: Element) {
    const part = element.getAttribute("data-rigged-ui");
    const testId = element.getAttribute("data-testid");
    return [
        element.tagName.toLowerCase(),
        part && `[data-rigged-ui="${part}"]`,
...
/**
 * Audits every rounded rigged-ui part below `root`. When a descendant corner
...
export function assertParallelRoundedCorners(root: ParentNode) {
    const elements = Array.from(root.querySelectorAll("[data-rigged-ui]"));
...
export function createRenderer(defaultOptions?: RendererOptions) {
    const renderer = createPlaywrightRenderer<JSX.Element>(
