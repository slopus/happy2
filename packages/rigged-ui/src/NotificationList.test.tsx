...
const kindColor: Record<NotificationKind, string> = {
    mention: "rgb(168, 155, 255)", // --rg-accent-strong
    thread_reply: "rgb(96, 165, 250)", // --rg-info
    direct_message: "rgb(110, 231, 183)", // --rg-success-strong
    reaction: "rgb(252, 211, 77)", // --rg-warning-strong
    call: "rgb(110, 231, 183)", // --rg-success-strong
    system: "rgb(165, 160, 176)", // --rg-text-secondary
    moderation: "rgb(252, 165, 165)", // --rg-danger-strong
    automation: "rgb(252, 211, 77)", // --rg-warning-strong
};
...
it("holds NotificationList geometry, row anatomy, typography, and optical alignment", async () => {
    ...
    expect(list.element.getAttribute("data-rigged-ui")).toBe("notification-list");
    ...
    expect(
        view.container.querySelector(
            '[data-testid="inbox"] [data-item-id="n3"] [data-rigged-ui="notification-unread"]',
        ),
        "read row has no unread dot",
    ).toBeNull();

    const dot = view.$(
        '[data-testid="inbox"] [data-item-id="n1"] [data-rigged-ui="notification-unread"]',
    );
    ...
    const avatar = view.$('[data-testid="inbox"] [data-item-id="n1"] [data-rigged-ui="avatar"]');
    expect(rel(avatar, row("n1"))).toEqual({ x: 36, y: 14, width: 36, height: 36 });

    /* ---- Corner kind badge: 18px raised chip with a 2px card ring ----- */

    const corner = view.$(
        '[data-testid="inbox"] [data-item-id="n1"] [data-rigged-ui="notification-kind"]',
    );
    ...
    const cornerGlyph = view.$(
        '[data-testid="inbox"] [data-item-id="n1"] [data-rigged-ui="notification-kind"] svg',
    );
    ...
    expect(
        view.container.querySelector(
            '[data-testid="inbox"] [data-item-id="n4"] [data-rigged-ui="avatar"]',
        ),
        "actor-less row has no avatar",
    ).toBeNull();
    const tile = view.$(
        '[data-testid="inbox"] [data-item-id="n4"] [data-rigged-ui="notification-kind"]',
    );
    ...
    expect(tile.computedStyles(["background-color", "border-radius", "color"])).toEqual({
        "background-color": "rgb(36, 34, 43)", // neutral tone → --rg-bg-raised
        "border-radius": "999px",
        color: kindColor.system,
    });
    // 16px glyph is an Icon tuned size (centroid ≤ 0.4px by construction).
    const tileGlyph = view.$(
        '[data-testid="inbox"] [data-item-id="n4"] [data-rigged-ui="notification-kind"] svg',
    );
    ...
    const body = view.$(
        '[data-testid="inbox"] [data-item-id="n1"] [data-rigged-ui="notification-body"]',
    );
    ...
    const text = view.$(
        '[data-testid="inbox"] [data-item-id="n1"] [data-rigged-ui="notification-text"]',
    );
    const textMetrics = text.textMetrics();
    expect(textMetrics.font.family).toBe("Rigged Figtree, system-ui, sans-serif");
    ...
    const actorLead = view.$(
        '[data-testid="inbox"] [data-item-id="n1"] [data-rigged-ui="notification-actor"]',
    );
    ...
    const readText = view.$(
        '[data-testid="inbox"] [data-item-id="n3"] [data-rigged-ui="notification-text"]',
    );
    expect(readText.textMetrics().font.weight).toBe("500");

    const context = view.$(
        '[data-testid="inbox"] [data-item-id="n1"] [data-rigged-ui="notification-context"]',
    );
    expect(context.textMetrics().font.size).toBe(12);
    expect(context.textMetrics().font.lineHeight).toBe(16);
    expect(context.computedStyle("color")).toBe("rgb(117, 112, 133)"); // --rg-text-muted
    expect((await context.visibleMetrics()).pixelCount, "context ink").toBeGreaterThan(0);

    /* ---- Time: mono, muted, right-aligned in the row ----------------- */

    const time = view.$(
        '[data-testid="inbox"] [data-item-id="n1"] [data-rigged-ui="notification-time"]',
    );
...
it("renders every notification kind with a centred kind glyph, plus tiles and the empty state", async () => {
    ...
    for (const item of allKinds) {
        const kind = view.$(
            `[data-testid="kinds"] [data-item-id="${item.id}"] [data-rigged-ui="notification-kind"]`,
        );
        ...
        const glyph = view.$(
            `[data-testid="kinds"] [data-item-id="${item.id}"] [data-rigged-ui="notification-kind"] svg`,
        );
    ...
    const tileBg: Record<string, string> = {
        "t-system": "rgb(36, 34, 43)", // neutral → --rg-bg-raised
        "t-moderation": "rgba(248, 113, 113, 0.13)", // --rg-danger-soft
        "t-automation": "rgba(251, 191, 36, 0.13)", // --rg-warning-soft
    };
    for (const item of tiles) {
        const tile = view.$(
            `[data-testid="tiles"] [data-item-id="${item.id}"] [data-rigged-ui="notification-kind"]`,
        );
        ...
        const glyph = view.$(
            `[data-testid="tiles"] [data-item-id="${item.id}"] [data-rigged-ui="notification-kind"] svg`,
        );
    ...
    const empty = view.$('[data-testid="empty"] [data-rigged-ui="notification-list-empty"]');
