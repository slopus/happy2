 ...
 * Button pair on the right, a `--rg-bg-code` well (radius 6) holding a mono
...
const computedFontFamily =
    server.browser === "webkit"
        ...
        : '"Rigged Figtree", system-ui, sans-serif';
const uiFamily = "Rigged Figtree, system-ui, sans-serif";
const monoFamily = "Rigged Mono, ui-monospace, monospace";

const SECRET = "rgd_demo_secret_0f2c1a7b4e51d0c8a6f3b27e"; // 40 chars → wraps at 296px
...
it("holds SecretReveal card, header, mono token, and warning-banner contract", async () => {
    ...
    const header = view.$('[data-testid="masked"] [data-rigged-ui="secret-reveal-header"]');
    ...
    const label = view.$('[data-testid="masked"] [data-rigged-ui="secret-reveal-label"]');
    ...
    const meta = view.$('[data-testid="masked"] [data-rigged-ui="secret-reveal-meta"]');
    ...
    const reveal = view.$('[data-testid="masked"] .rigged-secret-reveal__reveal');
    const copy = view.$('[data-testid="masked"] .rigged-secret-reveal__copy');
    ...
    const copyLabel = view.$(
        '[data-testid="masked"] .rigged-secret-reveal__copy [data-rigged-ui="button-label"]',
    );
    expect(copyLabel.textMetrics().text, "copy label").toBe("Copy");
    const copyGlyph = view.$('[data-testid="masked"] .rigged-secret-reveal__copy svg');
    ...
    const revealGlyph = view.$('[data-testid="masked"] .rigged-secret-reveal__reveal svg');
    ...
    const field = view.$('[data-testid="masked"] [data-rigged-ui="secret-reveal-field"]');
    ...
    const token = view.$('[data-testid="masked"] [data-rigged-ui="secret-reveal-token"]');
    ...
    const banner = view.$('[data-testid="masked"] [data-rigged-ui="banner"]');
    ...
    const bannerMsg = view.$('[data-testid="masked"] [data-rigged-ui="banner-message"]');
    expect(bannerMsg.textMetrics().text, "banner message").toBe(WARNING);

    // ---- Masked vs revealed: width stable, only height/colour change --------
    const revealedField = view.$('[data-testid="revealed"] [data-rigged-ui="secret-reveal-field"]');
    const revealedToken = view.$('[data-testid="revealed"] [data-rigged-ui="secret-reveal-token"]');
...
it("holds SecretReveal copied, minimal, and reveal/copy interaction states", async () => {
    ...
    const copiedButton = view.$('[data-testid="copied"] .rigged-secret-reveal__copy');
    expect(
    ...
    const copiedLabel = view.$(
        '[data-testid="copied"] .rigged-secret-reveal__copy [data-rigged-ui="button-label"]',
    );
    expect(copiedLabel.textMetrics().text, "copied label").toBe("Copied");
    const copiedGlyph = view.$('[data-testid="copied"] .rigged-secret-reveal__copy svg');
    ...
    expect(
        view.container.querySelector(
            '[data-testid="minimal"] [data-rigged-ui="secret-reveal-label"]',
        ),
        "no label",
    ).toBeNull();
    expect(
        view.container.querySelector(
            '[data-testid="minimal"] [data-rigged-ui="secret-reveal-meta"]',
        ),
        "no meta",
    ).toBeNull();
    expect(
        view.container.querySelector('[data-testid="minimal"] [data-rigged-ui="banner"]'),
        "no warning banner",
    ).toBeNull();
    const minimalHeader = view.$('[data-testid="minimal"] [data-rigged-ui="secret-reveal-header"]');
    expect(minimalHeader.bounds().height, "header collapses to action row").toBe(28);
    const minimalField = view.$('[data-testid="minimal"] [data-rigged-ui="secret-reveal-field"]');
    expect(minimalField.bounds().width, "minimal field width").toBe(266); // 300 - 32 - 2
    const minimalToken = view.$('[data-testid="minimal"] [data-rigged-ui="secret-reveal-token"]');
    ...
    const revealButton = view.$('[data-testid="minimal"] .rigged-secret-reveal__reveal')
        .element as HTMLButtonElement;
    const copyButton = view.$('[data-testid="minimal"] .rigged-secret-reveal__copy')
        .element as HTMLButtonElement;
