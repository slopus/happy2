...
it("holds AgentDesk geometry, colors, and typography in the 340px shell panel", async () => {
    ...
    const header = view.$('[data-rigged-ui="agent-desk-header"]');
    ...
    const spark = view.$(".rigged-agent-desk__spark");
    expect(spark.computedStyle("color")).toBe("rgb(139, 124, 247)");
    const sparkInk = await ink(view, ".rigged-agent-desk__spark", 23.5);
    ...
    const title = view.$('[data-rigged-ui="agent-desk-title"]');
    ...
    const titleInk = await ink(view, '[data-rigged-ui="agent-desk-title"]', 23.5);
    ...
    const count = view.$(".rigged-agent-desk__count");
    ...
    const badgeInk = await ink(view, ".rigged-agent-desk__count", 23.5);
    expect(Math.abs(badgeInk.dy)).toBeLessThanOrEqual(TEXT_TOLERANCE);

    // Body: scroll container below the header.
    const body = view.$('[data-rigged-ui="agent-desk-body"]');
    ...
    const tileOne = view.$(".rigged-agent-desk__run:nth-of-type(1)");
    const tileTwo = view.$(".rigged-agent-desk__run:nth-of-type(2)");
    ...
    expect(
        view.$('.rigged-agent-desk__run:nth-of-type(1) [data-rigged-ui="avatar"]').bounds(),
    ).toEqual({ x: 26, y: 72, width: 20, height: 20 });
    const runTitle = view.$(
        '.rigged-agent-desk__run:nth-of-type(1) [data-rigged-ui="agent-desk-run-title"]',
    );
    ...
    const eta = view.$(
        '.rigged-agent-desk__run:nth-of-type(1) [data-rigged-ui="agent-desk-run-eta"]',
    );
    ...
    const headSelector = '.rigged-agent-desk__run:nth-of-type(1) [data-rigged-ui="%"]';
    ...
    const detail = view.$('[data-rigged-ui="agent-desk-run-detail"]');
    expect(
    ...
    const track = view.$(
        '.rigged-agent-desk__run:nth-of-type(1) [data-rigged-ui="agent-desk-run-track"]',
    );
    ...
    const fill = view.$(
        '.rigged-agent-desk__run:nth-of-type(1) [data-rigged-ui="agent-desk-run-fill"]',
    );
    ...
    const queuedRows = desk.element.querySelectorAll('[data-rigged-ui="agent-desk-queued"]');
    expect(queuedRows.length).toBe(2);
    const queuedOne = view.$('[data-rigged-ui="agent-desk-queued"]');
    ...
    const clockSelector = '[data-rigged-ui="agent-desk-queued"] .rigged-agent-desk__row-icon';
    ...
    const queuedTitle = view.$(
        '[data-rigged-ui="agent-desk-queued"] [data-rigged-ui="agent-desk-row-title"]',
    );
    ...
    const queuedMeta = view.$(
        '[data-rigged-ui="agent-desk-queued"] [data-rigged-ui="agent-desk-row-meta"]',
    );
    ...
    const doneOne = view.$('[data-rigged-ui="agent-desk-done"]');
    ...
    const checkSelector = '[data-rigged-ui="agent-desk-done"] .rigged-agent-desk__row-icon';
    ...
    const doneTitle = view.$(
        '[data-rigged-ui="agent-desk-done"] [data-rigged-ui="agent-desk-row-title"]',
    );
    ...
    const doneMetaInk = await ink(
        view,
        '[data-rigged-ui="agent-desk-done"] [data-rigged-ui="agent-desk-row-meta"]',
        16,
    );
    ...
    const runTitlePart = view.$('[data-rigged-ui="agent-desk-run-title"]');
    const rowTitlePart = view.$('[data-rigged-ui="agent-desk-row-title"]');
    if (engine() === "chromium") {
...
async function expectDeskCentered(view: View, testId: string) {
    ...
    const spark = await ink(view, `${p} .rigged-agent-desk__spark`, 23.5);
    ...
    const queuedTitle = await ink(
        view,
        `${p} [data-rigged-ui="agent-desk-queued"] [data-rigged-ui="agent-desk-row-title"]`,
        18,
    );
    ...
    const queuedMeta = await ink(
        view,
        `${p} [data-rigged-ui="agent-desk-queued"] [data-rigged-ui="agent-desk-row-meta"]`,
        18,
    );
    ...
    const check = await ink(
        view,
        `${p} [data-rigged-ui="agent-desk-done"] .rigged-agent-desk__row-icon`,
        16,
    );
    ...
    const doneTitle = await ink(
        view,
        `${p} [data-rigged-ui="agent-desk-done"] [data-rigged-ui="agent-desk-row-title"]`,
        16,
    );
    expect(Math.abs(doneTitle.dy), `${testId} done title dy`).toBeLessThanOrEqual(TEXT_TOLERANCE);
    const doneMeta = await ink(
        view,
        `${p} [data-rigged-ui="agent-desk-done"] [data-rigged-ui="agent-desk-row-meta"]`,
        16,
    );
...
it("keeps ink optically centered at 280 and 400 widths and in a scrolling desk", async () => {
    ...
    const body = view.$('[data-testid="short"] [data-rigged-ui="agent-desk-body"]');
    expect(body.element.scrollHeight).toBeGreaterThan(body.element.clientHeight);
    const s = `[data-testid="short"]`;
    const spark = await ink(view, `${s} .rigged-agent-desk__spark`, 23.5);
    expect(Math.abs(spark.dy)).toBeLessThanOrEqual(ICON_TOLERANCE);
    const runTitle = await ink(view, `${s} [data-rigged-ui="agent-desk-run-title"]`, 10);
    expect(Math.abs(runTitle.dy)).toBeLessThanOrEqual(TEXT_TOLERANCE);
    const runEta = await ink(view, `${s} [data-rigged-ui="agent-desk-run-eta"]`, 10);
    ...
    const check = await ink(
        view,
        `${s} [data-rigged-ui="agent-desk-done"] .rigged-agent-desk__row-icon`,
        16,
    );
    ...
    const doneMeta = await ink(
        view,
        `${s} [data-rigged-ui="agent-desk-done"] [data-rigged-ui="agent-desk-row-meta"]`,
        16,
    );
...
it("handles custom icons, 1-char initials, and truncating labels", async () => {
    ...
    const branch = view.$('[data-rigged-ui="agent-desk-queued"] .rigged-agent-desk__row-icon');
    expect(branch.element.getAttribute("data-name")).toBe("branch");
    const branchInk = await ink(
        view,
        '[data-rigged-ui="agent-desk-queued"] .rigged-agent-desk__row-icon',
        18,
    );
    ...
    const doc = view.$('[data-rigged-ui="agent-desk-done"] .rigged-agent-desk__row-icon');
    ...
    const docInk = await ink(
        view,
        '[data-rigged-ui="agent-desk-done"] .rigged-agent-desk__row-icon',
        16,
    );
    ...
    const avatar = view.$('[data-rigged-ui="agent-desk-run"] [data-rigged-ui="avatar"]');
...
it("stays fluid, clamps progress, and scrolls overflowing content", async () => {
    ...
    expect(
        view.$('[data-testid="desk-narrow"] [data-rigged-ui="agent-desk-title"]').element
            .textContent,
    ).toBe("Codex desk");
    expect(
        view.$('[data-testid="desk-narrow"] .rigged-agent-desk__count').element.textContent,
    ).toBe("PAUSED");
    const narrowTile = view.$('[data-testid="desk-narrow"] [data-rigged-ui="agent-desk-run"]');
    expect(narrowTile.bounds()).toEqual({ x: 14, y: 60, width: 252, height: 44 });
    expect(narrow.element.querySelector('[data-rigged-ui="agent-desk-run-track"]')).toBeNull();
    expect(narrow.element.querySelector('[data-rigged-ui="agent-desk-run-eta"]')).toBeNull();
    expect(
        narrow.element.querySelectorAll('[data-rigged-ui="agent-desk-section-label"]').length,
    ).toBe(0);
    ...
    const wideTracks = view
        .$('[data-testid="desk-wide"]')
        .element.querySelectorAll('[data-rigged-ui="agent-desk-run-track"]');
    ...
    for (const [index, expected] of fillWidths.entries()) {
        const track = view.$(
            `[data-testid="desk-wide"] .rigged-agent-desk__run:nth-of-type(${index + 1}) [data-rigged-ui="agent-desk-run-track"]`,
        );
        expect(track.bounds().width, `track ${index}`).toBe(348);
        expect(track.height(), `track ${index}`).toBe(3);
        const fill = view.$(
            `[data-testid="desk-wide"] .rigged-agent-desk__run:nth-of-type(${index + 1}) [data-rigged-ui="agent-desk-run-fill"]`,
        );
    ...
    const scrollBody = view.$('[data-testid="desk-scroll"] [data-rigged-ui="agent-desk-body"]');
