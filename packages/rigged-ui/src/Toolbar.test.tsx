...
it(
    ...
    async () => {
        ...
        const root = (s: string) => view.$(`[data-testid="${s}"] [data-rigged-ui="toolbar"]`);
        const sel = (s: string, name: string) =>
            `[data-testid="${s}"] [data-rigged-ui="toolbar-${name}"]`;
       ...
       bottom must sit strictly inside the captured box. */
        ...
        expect(rFull.computedStyle("--rg-toolbar-title-optical-y")).toBe(titleOpticalY[engine()]);
        expect(rFull.computedStyle("--rg-toolbar-subtitle-optical-y")).toBe(
       ...
       the 0.75px composed-glyph ceiling on both axes. */
        ...
        const fullButtons = view.container.querySelectorAll(
            `${sel("s-full", "trailing")} [data-rigged-ui="button"]`,
        );
        expect(fullButtons.length, "s-full trailing has two buttons").toBe(2);
        const firstButton = view.$(`${sel("s-full", "trailing")} [data-rigged-ui="button"]`);
        ...
        const leadingButton = view.$(`${sel("s-leading", "leading")} [data-rigged-ui="button"]`);
