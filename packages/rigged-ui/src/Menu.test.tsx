...
async function iconCentroid(view: ReturnType<typeof createRenderer>, itemId: string) {
    const svg = view.$(
        `[data-testid="actions"] [data-item-id="${itemId}"] [data-rigged-ui="menu-item-icon"] svg`,
    );
...
it("holds Menu popover geometry, item rows, icon centroids, danger, and shortcuts", async () => {
    ...
    const list = view.$('[data-testid="actions"] [data-rigged-ui="menu-list"]');
    ...
    const separator = view.$('[data-testid="actions"] [data-rigged-ui="menu-separator"]');
    ...
    for (const id of rowIds) {
        const slot = view.$(
            `[data-testid="actions"] [data-item-id="${id}"] [data-rigged-ui="menu-item-icon"]`,
        );
    ...
    expect(
        view
            .$('[data-testid="actions"] [data-item-id="copy"] [data-rigged-ui="menu-item-icon"]')
            .computedStyle("color"),
    ...
    const label = (id: string) =>
        view.$(`[data-testid="actions"] [data-item-id="${id}"] [data-rigged-ui="menu-item-label"]`);
    ...
    expect(
        view
            .$('[data-testid="actions"] [data-item-id="delete"] [data-rigged-ui="menu-item-icon"]')
            .computedStyle("color"),
    ...
    for (const id of ["copy", "view", "edit", "delete"] as const) {
        const cap = view.$(
            `[data-testid="actions"] [data-item-id="${id}"] [data-rigged-ui="key-cap"]`,
        );
    ...
    expect(
        view.container.querySelector(
            '[data-testid="actions"] [data-item-id="star"] [data-rigged-ui="key-cap"]',
        ),
    ).toBeNull();
...
it("holds Menu section labels, disabled items, and text-only alignment", async () => {
    ...
    const menuLabels = view.container.querySelectorAll(
        '[data-testid="grouped"] [data-rigged-ui="menu-label"]',
    );
    expect(menuLabels.length).toBe(2);

    const sortBy = view.$('[data-testid="grouped"] [data-rigged-ui="menu-label"]');
    ...
    expect(
        view.container.querySelector('[data-testid="text"] [data-rigged-ui="menu-item-icon"]'),
    ).toBeNull();
    /* Labels sit flush on the 10px row padding when there is no gutter. */
    const rename = view.$(
        '[data-testid="text"] [data-item-id="rename"] [data-rigged-ui="menu-item-label"]',
    );
    ...
    const dupCap = view.$(
        '[data-testid="text"] [data-item-id="duplicate"] [data-rigged-ui="key-cap"]',
    );
