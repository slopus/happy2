...
const TEXT = "rgb(237, 234, 242)"; // --rg-text  #edeaf2
const SECONDARY = "rgb(165, 160, 176)"; // --rg-text-secondary #a5a0b0
const MUTED = "rgb(117, 112, 133)"; // --rg-text-muted #757085
const SURFACE = "rgb(28, 27, 34)"; // --rg-bg-surface #1c1b22
const HAIRLINE = "rgba(255, 255, 255, 0.07)"; // --rg-border
const ACCENT = "rgb(139, 124, 247)"; // --rg-accent #8b7cf7
...
it("holds PolicyControl card, sections, composed controls, and optical centering", async () => {
    ...
    const expiryTitle = view.$('[data-section="expiry"] [data-rigged-ui="policy-control-title"]');
    ...
    const expiryHelp = view.$('[data-section="expiry"] [data-rigged-ui="policy-control-help"]');
    ...
    const retentionTitle = view.$(
        '[data-section="retention"] [data-rigged-ui="policy-control-title"]',
    );
    expect(retentionTitle.textMetrics().text, "retention title").toBe("Message retention");
    const retentionHelp = view.$(
        '[data-section="retention"] [data-rigged-ui="policy-control-help"]',
    );
    ...
    await assertLegibleUnclipped(
        view,
        '[data-section="expiry"] [data-rigged-ui="policy-control-title"]',
    );
    await assertLegibleUnclipped(
        view,
        '[data-section="expiry"] [data-rigged-ui="policy-control-help"]',
    );

    // ---- Hairline rule between the two sections, with symmetric 20px gaps ----
    const rule = view.$('[data-rigged-ui="policy-control-rule"]');
    ...
    const expirySeg = view.$('[data-section="expiry"] [data-rigged-ui="segmented-control"]');
    ...
    expect(expirySeg.computedStyle("--rigged-segmented-index"), "expiry selected index").toBe("2");
    ...
    const expiryPill = view
        .$('[data-section="expiry"] [data-rigged-ui="segmented-control-pill"]')
        .bounds();
    ...
    expect(
        view
            .$(
                '[data-section="expiry"] [data-value="after_read"] [data-rigged-ui="segmented-control-label"]',
            )
            .computedStyle("color"),
    ...
    expect(
        view
            .$(
                '[data-section="expiry"] [data-value="none"] [data-rigged-ui="segmented-control-label"]',
            )
            .computedStyle("color"),
    ...
    for (const value of expiryValues) {
        expect(
            labelSymmetry(
                ...
                `[data-section="expiry"] [data-value="${value}"] [data-rigged-ui="segmented-control-label"]`,
            ),
    ...
    const timerRow = view.$(".rigged-policy-control__field--timer");
    ...
    const timerLabel = view.$(
        ".rigged-policy-control__field--timer [data-rigged-ui=form-row-label]",
    );
    ...
    const timerDesc = view.$(
        ".rigged-policy-control__field--timer [data-rigged-ui=form-row-description]",
    );
    ...
    const timerControl = view.$(
        ".rigged-policy-control__field--timer [data-rigged-ui=form-row-control]",
    );
    expect(
    ...
    const timerSelect = view.$(
        ".rigged-policy-control__field--timer [data-rigged-ui=select-control]",
    );
    expect(timerSelect.bounds().width, "timer select width").toBe(160);
    expect(timerSelect.bounds().height, "timer select height").toBe(36);
    expect(
        view.$(".rigged-policy-control__field--timer [data-rigged-ui=select-value]").textMetrics()
            .text,
        "timer select value",
    ).toBe("1 hour");
    const timerChevron = ".rigged-policy-control__field--timer [data-rigged-ui=select-chevron] svg";
    ...
    const scopeRow = view.$(".rigged-policy-control__field--scope");
    expect(scopeRow.bounds().width, "scope row width").toBe(CONTENT_W);
    expect(
        view.$(".rigged-policy-control__field--scope [data-rigged-ui=form-row-label]").textMetrics()
            .text,
        "scope label",
    ).toBe("Wait for all readers");
    const scopeControl = view.$(
        ".rigged-policy-control__field--scope [data-rigged-ui=form-row-control]",
    );
    expect(
        Math.abs(scopeControl.offsets().right),
        "scope control right-aligned",
    ).toBeLessThanOrEqual(0.1);
    const scopeTrack = view.$(".rigged-policy-control__field--scope [data-rigged-ui=switch-track]");
    const scopeThumb = view.$(".rigged-policy-control__field--scope [data-rigged-ui=switch-thumb]");
    ...
    const thumb = view.$(".rigged-policy-control__field--scope [data-rigged-ui=switch-thumb]");
    ...
    const retSeg = view.$('[data-section="retention"] [data-rigged-ui="segmented-control"]');
    ...
    expect(retSeg.computedStyle("--rigged-segmented-index"), "retention selected index").toBe("2");
    expect(
        view
            .$(
                '[data-section="retention"] [data-value="duration"] [data-rigged-ui="segmented-control-label"]',
            )
            .computedStyle("color"),
    ...
    expect(
        view
            .$(".rigged-policy-control__field--retention [data-rigged-ui=select-value]")
            .textMetrics().text,
        "retention select value",
    ).toBe("30 days");
    const retChevron =
        ".rigged-policy-control__field--retention [data-rigged-ui=select-chevron] svg";
...
it("reveals conditional rows only for the matching expiry and retention modes", async () => {
    ...
    expect(has("p-off", '[data-rigged-ui="select"]'), "off has no Select").toBe(false);
    expect(has("p-off", '[data-rigged-ui="switch"]'), "off has no Switch").toBe(false);
    expect(has("p-off", '[data-section="retention"]'), "off has no retention").toBe(false);
    expect(has("p-off", '[data-rigged-ui="policy-control-rule"]'), "off has no rule").toBe(false);
    expect(
        view
            .$('[data-testid="p-off"] [data-rigged-ui="segmented-control"]')
            .computedStyle("--rigged-segmented-index"),
        "off segmented index",
    ).toBe("0");

    // After sending: timer Select present, scope Switch absent.
    expect(
        has("p-send", '[data-section="expiry"] [data-rigged-ui="select"]'),
        "send has timer Select",
    ).toBe(true);
    expect(has("p-send", '[data-rigged-ui="switch"]'), "send has no scope Switch").toBe(false);
    ...
    expect(
        has("p-send", '[data-section="retention"] [data-rigged-ui="select"]'),
        "send retention has no duration Select",
    ).toBe(false);
    expect(
        view
            .$(
                '[data-testid="p-send"] [data-section="retention"] [data-rigged-ui="segmented-control"]',
            )
            .computedStyle("--rigged-segmented-index"),
    ...
    expect(
        has("p-read", '[data-section="expiry"] [data-rigged-ui="select"]'),
        "read has timer Select",
    ).toBe(true);
    expect(
        has("p-read", '[data-section="expiry"] [data-rigged-ui="switch"]'),
        "read has scope Switch",
    ).toBe(true);
    expect(
        has("p-read", '[data-section="retention"] [data-rigged-ui="select"]'),
        "read has retention duration Select",
    ).toBe(true);
    // The scope switch reflects all_readers = checked.
    expect(
        view
            .$('[data-testid="p-read"] [data-rigged-ui="switch"]')
