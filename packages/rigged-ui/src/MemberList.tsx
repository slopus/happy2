...
function MemberRow(props: {
    ...
    return (
        <li
            class="rigged-member-list__row"
            data-member-id={member().id}
            data-presence={member().presence ?? "offline"}
            data-rigged-ui="member-row"
            data-role={member().role}
        >
            <Avatar
                class="rigged-member-list__avatar"
                ...
                tone={member().tone}
            />
            <span class="rigged-member-list__identity" data-rigged-ui="member-identity">
                <span class="rigged-member-list__name" data-rigged-ui="member-name">
                    {member().name}
                </span>
                <Show when={subtitle()}>
                    <span class="rigged-member-list__subtitle" data-rigged-ui="member-subtitle">
                        {subtitle()}
                    </span>
                </Show>
            </span>
            <Badge class="rigged-member-list__role" label={role().label} variant={role().variant} />
            <Show when={trailing()}>
                <span class="rigged-member-list__trailing" data-rigged-ui="member-trailing">
                    {trailing()}
                </span>
            </Show>
        </li>
...
export function MemberList(props: MemberListProps) {
    ...
    return (
        <ul
            {...rest}
            class={["rigged-member-list", local.class].filter(Boolean).join(" ")}
            data-rigged-ui="member-list"
            style={local.style}
        >
