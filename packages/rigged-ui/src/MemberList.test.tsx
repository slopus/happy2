...
const fontFamily = "Rigged Figtree, system-ui, sans-serif";
...
it("holds MemberList geometry, typography, role badges, and optical alignment", async () => {
    ...
    view.render(
        () => (
            <div style={{ background: "var(--rg-bg-surface)", width: "100%" }}>
                <MemberList
                    actionLabel="Message"
                    data-testid="roster"
     ...
     * first row has no divider; every following row draws --rg-border. */
    ...
    for (const id of rowIds) {
        const avatar = view.$(sel(`[data-member-id="${id}"] [data-rigged-ui="avatar"]`));
    ...
    expect(
        view.$(sel('[data-member-id="ada"] [data-rigged-ui="avatar-presence"]')).bounds().width,
    ).toBe(8);
    expect(
        view.container.querySelector(
            sel('[data-member-id="grace"] [data-rigged-ui="avatar-presence"]'),
        ),
    ).toBeNull();
    ...
    for (const id of rowIds) {
        const identity = view.$(sel(`[data-member-id="${id}"] [data-rigged-ui="member-identity"]`));
    ...
    for (const id of rowIds) {
        const name = view.$(sel(`[data-member-id="${id}"] [data-rigged-ui="member-name"]`));
    ...
    for (const id of rowIds) {
        const sub = view.$(sel(`[data-member-id="${id}"] [data-rigged-ui="member-subtitle"]`));
    ...
    for (const id of rowIds) {
        const badge = view.$(sel(`[data-member-id="${id}"] [data-rigged-ui="badge"]`));
    ...
    for (const id of rowIds) {
        const trailing = view.$(sel(`[data-member-id="${id}"] [data-rigged-ui="member-trailing"]`));
        expect(trailing.offsets().right, `${id} trailing right`).toBe(12);
        const button = view.$(
            sel(
                `[data-member-id="${id}"] [data-rigged-ui="member-trailing"] [data-rigged-ui="button"]`,
            ),
        );
    ...
    (
        view.$(
            sel(
                '[data-member-id="linus"] [data-rigged-ui="member-trailing"] [data-rigged-ui="button"]',
            ),
...
it("holds MemberList trailing variants, minimal rows, and role colors", async () => {
    ...
    for (const [id, name] of [
        ...
        const trailing = view.$(
            `[data-testid="menu"] [data-member-id="${id}"] [data-rigged-ui="member-trailing"]`,
        );
        expect(trailing.offsets().right, `${id} menu trailing right`).toBe(12);
        const button = view.$(
            `[data-testid="menu"] [data-member-id="${id}"] [data-rigged-ui="member-trailing"] [data-rigged-ui="button"]`,
        );
        ...
        expect(button.bounds().height, id).toBe(28);
        const icon = view.$(
            `[data-testid="menu"] [data-member-id="${id}"] [data-rigged-ui="member-trailing"] svg`,
        );
    ...
    const adaBadge = view.$('[data-testid="menu"] [data-member-id="ada"] [data-rigged-ui="badge"]');
    ...
    expect(
        view.container.querySelector(
            '[data-testid="plain"] [data-member-id="solo"] [data-rigged-ui="member-trailing"]',
        ),
        "solo has no trailing control",
    ).toBeNull();
    expect(
        view.container.querySelector(
            '[data-testid="plain"] [data-member-id="solo"] [data-rigged-ui="member-subtitle"]',
        ),
    ...
    const soloIdentity = view.$(
        '[data-testid="plain"] [data-member-id="solo"] [data-rigged-ui="member-identity"]',
    );
    ...
    const soloBadge = view.$(
        '[data-testid="plain"] [data-member-id="solo"] [data-rigged-ui="badge"]',
    );
    ...
    const imgAvatar = view.$(
        '[data-testid="plain"] [data-member-id="img"] [data-rigged-ui="avatar-image"]',
    );
    ...
    expect(
        view.container.querySelector(
            '[data-testid="plain"] [data-member-id="img"] [data-rigged-ui="avatar-initials"]',
        ),
    ).toBeNull();
    const imgBadge = view.$(
        '[data-testid="plain"] [data-member-id="img"] [data-rigged-ui="badge"]',
    );
