...
const dockerfilePlaceholder = "FROM rigged/agent-base:latest\n# Add the tools your agents need…";
...
export function AgentImagePanel(props: AgentImagePanelProps) {
    ...
    const rows = createMemo<DataTableRow[]>(() =>
        local.images.map((image) => {
            ...
            return {
                ...
                cells: {
                    name: (
                        <Box class="rigged-agent-image-panel__name">
                            <Box class="rigged-agent-image-panel__name-head">
                                <span class="rigged-agent-image-panel__name-text">
                                    {image.name}
                                </span>
                            ...
                            <Show when={subline}>
                                {(line) => (
                                    <span
                                        class="rigged-agent-image-panel__subline"
                    ...
                    status: (
                        <Box class="rigged-agent-image-panel__status">
                            <Badge
                            ...
                            <Show when={image.status === "building"}>
                                <Box
                                    ...
                                    class="rigged-agent-image-panel__progress"
                                    role="progressbar"
                                >
                                    <span class="rigged-agent-image-panel__progress-track">
                                        <span
                                            class="rigged-agent-image-panel__progress-fill"
                                            style={{ width: `${progressValue(image.progress)}%` }}
                                        />
                                    </span>
                                    <span class="rigged-agent-image-panel__progress-value">
    ...
    const rowActions = (row: DataTableRow): JSX.Element => {
        ...
        return (
            <Box
                class="rigged-agent-image-panel__row-actions"
    ...
    return (
        <Box
            {...rest}
            class={["rigged-agent-image-panel", local.class].filter(Boolean).join(" ")}
            data-rigged-ui="agent-image-panel"
            style={local.style}
        >
            <Box class="rigged-agent-image-panel__header">
                <Box class="rigged-agent-image-panel__heading">
                    <span class="rigged-agent-image-panel__title">{title()}</span>
                    <Show when={local.subtitle}>
                        <span class="rigged-agent-image-panel__subtitle">{local.subtitle}</span>
                    </Show>
                </Box>
                <Box class="rigged-agent-image-panel__actions">
            ...
            <Show when={local.createOpen}>
                <Box
                    class="rigged-agent-image-panel__overlay"
                    data-rigged-ui="agent-image-panel-overlay"
                    onClick={() => local.onCloseCreate?.()}
                >
                    <Box onClick={(event) => event.stopPropagation()}>
                        <Modal
                            footer={
                                <Box class="rigged-agent-image-panel__modal-actions">
                        ...
                        >
                            <Box class="rigged-agent-image-panel__form">
                                <FormRow
                                    control={
                                        <TextField
                                            fullWidth
