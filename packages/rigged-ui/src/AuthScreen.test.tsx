...
it("holds AuthScreen split geometry, panel layout, typography, and optical brand glyph", async () => {
    ...
    const hero = view.$('[data-rigged-ui="auth-hero"]');
    const panel = view.$('[data-rigged-ui="auth-panel"]');
    ...
    const brand = view.$('[data-rigged-ui="auth-brand"]');
    const mark = view.$('[data-rigged-ui="auth-mark"]');
    const brandName = view.$('[data-rigged-ui="auth-brand-name"]');
    ...
    const markGlyph = await glyphDrift(
        view,
        '[data-rigged-ui="auth-mark"]',
        '[data-rigged-ui="auth-mark"] svg',
    );
    ...
    const content = view.$('[data-rigged-ui="auth-content"]');
    const kicker = view.$('[data-rigged-ui="auth-kicker"]');
    const title = view.$('[data-rigged-ui="auth-title"]');
    const copy = view.$('[data-rigged-ui="auth-copy"]');
    const form = view.$('[data-rigged-ui="auth-form"]');
    ...
    const footer = view.$('[data-rigged-ui="auth-footer"]');
...
it("holds AuthScreen loading, generated-image hero, custom mark, and minimal forms", async () => {
    ...
    const loader = view.$('[data-testid="loading"] [data-rigged-ui="auth-loader"]');
    expect(loader.computedStyles(["align-items", "display"])).toEqual({
        "align-items": "center",
        display: "flex",
    });

    const spinner = view.$('[data-testid="loading"] [data-rigged-ui="auth-spinner"]');
    ...
    const loadingLabel = view.$('[data-testid="loading"] [data-rigged-ui="auth-loading-label"]');
    ...
    const imageHero = view.$('[data-testid="image"] [data-rigged-ui="auth-hero"]');
    ...
    const customMark = view.$('[data-testid="image"] [data-rigged-ui="auth-mark"]');
    ...
    expect(
        minimal.element.querySelector('[data-rigged-ui="auth-brand"]'),
        "no brand mast",
    ).toBeNull();
    expect(minimal.element.querySelector('[data-rigged-ui="auth-kicker"]'), "no kicker").toBeNull();
    expect(minimal.element.querySelector('[data-rigged-ui="auth-copy"]'), "no copy").toBeNull();
    expect(minimal.element.querySelector('[data-rigged-ui="auth-footer"]'), "no footer").toBeNull();
    const minimalTitle = view.$('[data-testid="minimal"] [data-rigged-ui="auth-title"]');
    ...
    expect(view.$('[data-testid="minimal"] [data-rigged-ui="auth-panel"]').width()).toBe(480);
