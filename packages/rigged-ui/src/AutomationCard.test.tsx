...
const computedFontFamily =
    server.browser === "webkit"
        ...
        : `"Rigged Figtree", system-ui, sans-serif`;
const uiFont = "Rigged Figtree, system-ui, sans-serif";
const monoFont = "Rigged Mono, ui-monospace, monospace";
...
it("holds AutomationCard layout, geometry, typography, badge colors, and states", async () => {
    ...
    expect(card.computedStyle("--rg-automation-name-y"), "name-y").toBe(nameY[engine()]);
    expect(card.computedStyle("--rg-automation-detail-y"), "detail-y").toBe(detailY[engine()]);
    expect(card.computedStyle("--rg-automation-meta-y"), "meta-y").toBe(metaY[engine()]);

    /* — row rhythm: header 20 / flow 18 / detail 18 / footer 28 on a 12px gap — */
    expect(a('[data-rigged-ui="automation-card-header"]').bounds()).toEqual({
        x: 33,
        y: 33,
        width: 366,
        height: 20,
    });
    expect(a('[data-rigged-ui="automation-card-flow"]').bounds()).toEqual({
        x: 33,
        y: 65,
        width: 366,
        height: 18,
    });
    expect(a('[data-rigged-ui="automation-card-detail"]').bounds()).toEqual({
        x: 33,
        y: 95,
        width: 366,
        height: 18,
    });
    expect(a('[data-rigged-ui="automation-card-footer"]').bounds()).toEqual({
        x: 33,
        y: 125,
        width: 366,
        height: 28,
    ...
    const name = a('[data-rigged-ui="automation-card-name"]');
    ...
    const triggerBadge = a('[data-rigged-ui="automation-card-trigger"] [data-rigged-ui="badge"]');
    ...
    const actionBadge = a('[data-rigged-ui="automation-card-action"] [data-rigged-ui="badge"]');
    ...
    const arrow = a('[data-rigged-ui="automation-card-arrow"] svg');
    ...
    const triggerLabel = a('[data-rigged-ui="automation-card-trigger-label"]');
    ...
    expect(a('[data-rigged-ui="automation-card-action-label"]').element.textContent).toBe(
        "Post to #general",
    );

    /* — footer: mono meta run on the left, 28px run-now button on the right — */
    const last = a('[data-rigged-ui="automation-card-last"]');
    ...
    expect(a('[data-rigged-ui="automation-card-next"]').element.textContent).toBe("Next in 22h");
    const runWrap = a('[data-rigged-ui="automation-card-run"]');
    expect(runWrap.offsets().right, "run-now right-pinned").toBeCloseTo(0, 3);
    const runBtn = a('[data-rigged-ui="automation-card-run"] [data-rigged-ui="button"]');
    ...
    expect(
        b('[data-rigged-ui="automation-card-trigger"] [data-rigged-ui="badge"]').computedStyles([
            "background-color",
            "color",
        ]),
    ).toEqual({ "background-color": badgeColors.warning.bg, color: badgeColors.warning.fg });
    expect(
        b('[data-rigged-ui="automation-card-action"] [data-rigged-ui="badge"]').computedStyles([
            "background-color",
            "color",
        ]),
    ).toEqual({ "background-color": badgeColors.danger.bg, color: badgeColors.danger.fg });
    const banner = b('[data-rigged-ui="banner"]');
    ...
    const bannerMsg = b('[data-rigged-ui="banner-message"]');
    ...
    expect(c('[data-rigged-ui="switch"]').element.getAttribute("aria-checked"), "C off").toBe(
        "false",
    );
    expect(
        c('[data-rigged-ui="switch-track"]').computedStyle("background-color"),
        "C track off",
    ).toBe(OFF_TRACK);
    expect(
        c('[data-rigged-ui="automation-card-trigger"] [data-rigged-ui="badge"]').computedStyle(
            "color",
        ),
    ).toBe(badgeColors.accent.fg);
    expect(
        c('[data-rigged-ui="automation-card-action"] [data-rigged-ui="badge"]').computedStyle(
            "color",
        ),
    ).toBe(badgeColors.info.fg);
    // Minimal card renders no detail / error / footer rows.
    expect(cCard.element.querySelector('[data-rigged-ui="automation-card-detail"]')).toBeNull();
    expect(cCard.element.querySelector('[data-rigged-ui="automation-card-error"]')).toBeNull();
    expect(cCard.element.querySelector('[data-rigged-ui="automation-card-footer"]')).toBeNull();
...
it("centers ink optically in every card row and glyph", async () => {
    ...
    const header = a('[data-rigged-ui="automation-card-header"]');
    const nameInk = await inkCenter(a('[data-rigged-ui="automation-card-name"]'), "name");
    expect(
    ...
    const arrow = a('[data-rigged-ui="automation-card-arrow"]');
    ...
    const detail = a('[data-rigged-ui="automation-card-detail"]');
    const detailInk = await inkCenter(
        a('[data-rigged-ui="automation-card-trigger-label"]'),
        "detail",
    );
    expect(
    ...
    const meta = a('[data-rigged-ui="automation-card-meta"]');
    const lastEl = a('[data-rigged-ui="automation-card-last"]');
    const nextEl = a('[data-rigged-ui="automation-card-next"]');
    ...
    const thumb = a('[data-rigged-ui="switch-thumb"]');
    ...
    const triggerLabelInk = await inkCenter(
        a('[data-rigged-ui="automation-card-trigger"] [data-rigged-ui="badge-label"]'),
        "badge-label",
    );
    const triggerBadge = a('[data-rigged-ui="automation-card-trigger"] [data-rigged-ui="badge"]');
    expect(
    ...
    const runLabelInk = await inkCenter(
        a('[data-rigged-ui="automation-card-run"] [data-rigged-ui="button-label"]'),
        "run-label",
    );
    const runBtn = a('[data-rigged-ui="automation-card-run"] [data-rigged-ui="button"]');
