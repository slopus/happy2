...
it("holds Tabs dimensions, typography, colors, and the active underline for every size", async () => {
    ...
    for (const size of sizes) {
        ...
        const bar = view.$(`.rigged-tabs[data-size="${size}"]`);
        ...
        for (const tab of inboxTabs) {
            ...
            const el = view.$(`.rigged-tabs[data-size="${size}"] [data-tab-id="${tab.id}"]`);
            ...
            const label = view.$(
                `.rigged-tabs[data-size="${size}"] [data-tab-id="${tab.id}"] [data-rigged-ui="tab-label"]`,
            );
        ...
        const plain = view.$(
            `.rigged-tabs[data-size="${size}"] [data-tab-id="reactions"] [data-rigged-ui="tab-label"]`,
        );
        ...
        const activeTab = view.$(`.rigged-tabs[data-size="${size}"] [data-tab-id="unread"]`);
        const underline = view.$(
            `.rigged-tabs[data-size="${size}"] [data-tab-id="unread"] [data-rigged-ui="tab-underline"]`,
        );
        ...
        for (const id of ["all", "mentions", "threads", "reactions"]) {
            expect(
                view
                    .$(`.rigged-tabs[data-size="${size}"] [data-tab-id="${id}"]`)
                    .element.querySelector('[data-rigged-ui="tab-underline"]'),
        ...
        expect(
            view.$(`.rigged-tabs[data-size="${size}"]`).element.querySelectorAll("[data-active]")
                .length,
            `${size} single active`,
        ).toBe(1);
        ...
        for (const id of ["all", "mentions"]) {
            const iconBox = view.$(
                `.rigged-tabs[data-size="${size}"] [data-tab-id="${id}"] [data-rigged-ui="tab-icon"]`,
            );
            const tab = view.$(`.rigged-tabs[data-size="${size}"] [data-tab-id="${id}"]`);
        ...
        for (const { id, tone } of badgeCases) {
            const tab = view.$(`.rigged-tabs[data-size="${size}"] [data-tab-id="${id}"]`);
            const label = view.$(
                `.rigged-tabs[data-size="${size}"] [data-tab-id="${id}"] [data-rigged-ui="tab-label"]`,
            );
            const badge = view.$(
                `.rigged-tabs[data-size="${size}"] [data-tab-id="${id}"] [data-rigged-ui="count-badge"]`,
            );
    ...
    for (const size of sizes) {
        ...
        for (const id of ["all", "mentions"]) {
            const svg = view.$(
                `.rigged-tabs[data-size="${size}"] [data-tab-id="${id}"] [data-rigged-ui="tab-icon"] svg`,
            );
        ...
        for (const id of ["unread", "mentions"]) {
            const badgeInk = view.$(
                `.rigged-tabs[data-size="${size}"] [data-tab-id="${id}"] [data-rigged-ui="count-badge-label"]`,
            );
            expect(
...
it("holds Tabs arity, active sweep, and count badges", async () => {
    ...
    for (const [selector, count, activeId] of [
        ['.rigged-tabs [data-tab-id="a"]', 2, "a"],
        ['.rigged-tabs [data-tab-id="members"]', 4, "members"],
    ] as const) {
        const bar = view.$(selector).element.closest(".rigged-tabs")!;
        expect(bar.querySelectorAll('[data-rigged-ui="tab"]').length, `${activeId} arity`).toBe(
            count,
        );
        expect(
            bar.querySelectorAll('[data-rigged-ui="tab-underline"]').length,
            `${activeId} one underline`,
        ).toBe(1);
        expect(
            bar
                .querySelector(`[data-tab-id="${activeId}"] [data-rigged-ui="tab-underline"]`)
    ...
    const twoBar = view.$('[data-tab-id="a"]').element.closest(".rigged-tabs")!;
    const b9 = twoBar.querySelector('[data-tab-id="b"] [data-rigged-ui="count-badge"]');
    expect((b9 as HTMLElement).getBoundingClientRect().width, "single-digit badge").toBe(18);
    const adminBar = view.$('[data-tab-id="members"]').element.closest(".rigged-tabs")!;
    const b128 = adminBar.querySelector('[data-tab-id="members"] [data-rigged-ui="count-badge"]');
    expect(
    ...
    for (const active of ["all", "mentions", "reactions"] as const) {
        ...
        const underline = view.$(
            `[data-tab-id="${active}"][aria-selected="true"] [data-rigged-ui="tab-underline"]`,
        );
