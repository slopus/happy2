...
const row = (id: string) => `[data-rigged-ui="data-table-body"] [data-row-id="${id}"]`;
const nameCell = (id: string) => `${row(id)} [data-column-id="name"]`;
const statusCell = (id: string) => `${row(id)} [data-column-id="status"]`;
const statusBadge = (id: string) => `${statusCell(id)} [data-rigged-ui="badge"]`;
const defaultCell = (id: string) =>
    `${row(id)} [data-column-id="default"] [data-rigged-ui="data-table-cell"]`;
const actionsCell = (id: string) => `${row(id)} [data-rigged-ui="data-table-actions"]`;
...
it("holds AgentImagePanel layout, status mapping, the default marker, and row actions", async () => {
    ...
    const title = view.$(".rigged-agent-image-panel__title");
    ...
    expect(view.$(".rigged-agent-image-panel__subtitle").computedStyle("color")).toBe(
        "rgb(117, 112, 133)",
    );
    ...
    const actions = view.$(".rigged-agent-image-panel__actions");
    ...
    const defaultBadge = view.$(`${defaultCell("img-default")} [data-rigged-ui="badge"]`);
    ...
    for (const id of ["img-minimal", "img-building", "img-pending", "img-failed"]) {
        ...
        expect(
            view.$(defaultCell(id)).element.querySelector('[data-rigged-ui="badge"]'),
    ...
    const glyph = await inkDrift(
        view,
        `${defaultCell("img-default")} [data-rigged-ui="badge-icon"]`,
        `${defaultCell("img-default")} [data-rigged-ui="badge-icon"] svg`,
    );
...
it("shows build progress, the last log line, and opens a row on click", async () => {
    ...
    const track = view.$(`${statusCell("img-building")} .rigged-agent-image-panel__progress-track`);
    const fill = view.$(`${statusCell("img-building")} .rigged-agent-image-panel__progress-fill`);
    ...
    expect(
        view.$(`${statusCell("img-building")} .rigged-agent-image-panel__progress-value`).element
            .textContent,
    ).toBe("62%");
    ...
    const buildingSub = view.$(`${nameCell("img-building")} .rigged-agent-image-panel__subline`);
    ...
    const failedSub = view.$(`${nameCell("img-failed")} .rigged-agent-image-panel__subline`);
    ...
    expect(
        view.$(nameCell("img-minimal")).element.querySelector(".rigged-agent-image-panel__subline"),
...
it("renders the create overlay with controlled inputs and submit gating", async () => {
            ...
            () => (
                ...
                >
                    <AgentImagePanel
                        ...
                        draftDockerfile="FROM rigged/agent-base:latest"
    ...
    const overlay = view.$('[data-testid="empty-draft"] .rigged-agent-image-panel__overlay');
    expect(
    ...
    expect(
        overlay.element.querySelector('[data-rigged-ui="modal-dialog"]'),
        "modal present",
    ).not.toBeNull();
    ...
    expect(
        view.$('[data-testid="filled-draft"] [data-rigged-ui="banner"]').element.textContent,
...
it("shows loading, error, and empty affordances", async () => {
    ...
    expect(
        view.$('[data-testid="loading"]').element.querySelector('[data-rigged-ui="empty-state"]'),
        "loading empty-state",
    ).not.toBeNull();
    expect(
        view.$('[data-testid="loading"]').element.querySelector('[data-rigged-ui="data-table"]'),
        "no table while loading",
    ).toBeNull();

    // Error: a danger banner replaces the table entirely.
    const errorBanner = view.$('[data-testid="error"] [data-rigged-ui="banner"]');
    ...
    expect(
        view.$('[data-testid="error"]').element.querySelector('[data-rigged-ui="data-table"]'),
        "no table on error",
    ).toBeNull();

    // Empty (loaded, zero images): a table whose empty slot invites the first image.
    const emptySlot = view.$('[data-testid="empty"] [data-rigged-ui="data-table-empty"]');
...
function modalFooterButton(view: View, testId: string, label: string): HTMLButtonElement {
    const buttons = view
        .$(`[data-testid="${testId}"] [data-rigged-ui="modal-footer"]`)
