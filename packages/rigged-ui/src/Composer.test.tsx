...
it("holds Composer geometry, colors, and typography", async () => {
    ...
    const textarea = view.$(
        '[data-testid="composer-default"] [data-rigged-ui="composer-textarea"]',
    );
    ...
    const firstLineDrift = async (testid: string) => {
        const wrapperSelector = `[data-testid="${testid}"] [data-rigged-ui="composer-input"]`;
        const areaSelector = `[data-testid="${testid}"] [data-rigged-ui="composer-textarea"]`;
    ...
    const sendFilled = view.$('[data-testid="composer-filled"] .rigged-composer__send');
    ...
    const hint = view.$('[data-testid="composer-default"] [data-rigged-ui="composer-hint"]');
    ...
    const hintDrift = await centroidDrift(
        view,
        '[data-testid="composer-default"] [data-rigged-ui="composer-toolbar"]',
        '[data-testid="composer-default"] [data-rigged-ui="composer-hint"]',
    );
    ...
    const multiline = view.$(
        '[data-testid="composer-multiline"] [data-rigged-ui="composer-textarea"]',
    );
    ...
    const overflow = view.$(
        '[data-testid="composer-overflow"] [data-rigged-ui="composer-textarea"]',
    );
    ...
    const contextRow = view.$('[data-testid="composer-chips"] [data-rigged-ui="composer-context"]');
    expect(contextRow.offsets().top).toBe(1);
    expect(contextRow.bounds().height).toBe(32);
    const firstChip = view.$(
        '[data-testid="composer-chips"] [data-rigged-ui="context-chips-chip"]',
    );
    ...
    const disabledArea = view.$(
        '[data-testid="composer-disabled"] [data-rigged-ui="composer-textarea"]',
    );
    ...
    expect(
        (
            view.$('[data-testid="composer-disabled"] .rigged-composer__send')
...
it("holds ContextChips and MentionPicker geometry and colors", async () => {
        ...
        .render(
            () => (
                <>
                    ...
                    <style>
                        {`[data-testid="chips-baseline"] [data-rigged-ui="context-chips-chip"] {
    ...
    const chip = view.$('[data-testid="chips"] [data-rigged-ui="context-chips-chip"]');
    ...
    const chips = Array.from(
        view.container.querySelectorAll(
            '[data-testid="chips"] [data-rigged-ui="context-chips-chip"]',
        ),
    );
    ...
    for (const kind of ["file", "run", "thread"] as const) {
        ...
        const iconSelector = `${chipSelector} [data-rigged-ui="context-chips-icon"]`;
    ...
    const chipText = view.$('[data-testid="chips"] [data-rigged-ui="context-chips-text"]');
    ...
    const chipDetail = view.$('[data-testid="chips"] [data-rigged-ui="context-chips-detail"]');
    ...
    const chipsLabel = view.$('[data-testid="chips"] [data-rigged-ui="context-chips-label"]');
    expect(
    ...
    expect(chipsLabel.computedStyle("font-family")).toContain("Rigged Mono");
    ...
    const baselineText = await inkSpan(
        view,
        baselineChip,
        `${baselineChip} [data-rigged-ui="context-chips-text"]`,
    );
    const baselineDetail = await inkSpan(
        view,
        baselineChip,
        `${baselineChip} [data-rigged-ui="context-chips-detail"]`,
    );
    ...
    const labelDrift = await centroidDrift(
        view,
        '[data-testid="chips"]',
        '[data-testid="chips"] [data-rigged-ui="context-chips-label"]',
    );
    ...
    const removeButtons = Array.from(
        view.container.querySelectorAll(
            '[data-testid="chips"] [data-rigged-ui="context-chips-remove"]',
        ),
    );
    ...
    const removeSelector = '[data-testid="chips"] [data-rigged-ui="context-chips-remove"]';
    ...
    expect(
        view.container.querySelectorAll(
            '[data-testid="chips-readonly"] [data-rigged-ui="context-chips-remove"]',
        ).length,
    ).toBe(0);

    // Readonly chips keep the same lanes without the remove affordance.
    expect(
        view.$('[data-testid="chips-readonly"] [data-rigged-ui="context-chips-chip"]').bounds()
            .height,
    ).toBe(24);
    ...
    const header = view.$('[data-testid="picker"] [data-rigged-ui="mention-picker-header"]');
    ...
    const rows = Array.from(
        view.container.querySelectorAll(
            '[data-testid="picker"] [data-rigged-ui="mention-picker-row"]',
        ),
    );
    expect(rows.length).toBe(3);
    const row = view.$('[data-testid="picker"] [data-rigged-ui="mention-picker-row"]');
    expect(row.bounds().width).toBe(306);
    expect(row.bounds().height).toBe(44);
    const avatar = view.$('[data-testid="picker"] [data-rigged-ui="avatar"]');
    expect(avatar.bounds().width).toBe(28);
    expect(avatar.offsets().top).toBe(8);
    expect(avatar.offsets().left).toBe(8);
    expect(
        (await view.$('[data-testid="picker"] [data-rigged-ui="avatar-initials"]').visibleMetrics())
            .pixelCount,
    ).toBeGreaterThan(0);
    ...
    const meta = view.$(`${codexRow} [data-rigged-ui="mention-picker-meta"]`);
    expect(meta.bounds().height).toBe(32);
    expect(meta.offsets().top).toBe(6);
    const name = view.$('[data-testid="picker"] [data-rigged-ui="mention-picker-name"]');
    ...
    const metaSelector = `${codexRow} [data-rigged-ui="mention-picker-meta"]`;
    const nameSelector = `${codexRow} [data-rigged-ui="mention-picker-name"]`;
    const descriptionSelector = `${codexRow} [data-rigged-ui="mention-picker-description"]`;
    ...
    const activeRow = view.$(
        '[data-testid="picker"] [data-rigged-ui="mention-picker-row"][data-active]',
    );
    ...
    const readyBadge = view.$(
        '[data-testid="picker"] [data-agent-id="codex"] [data-rigged-ui="badge"]',
    );
    ...
    const badgeDrift = await centroidDrift(view, codexRow, `${codexRow} [data-rigged-ui="badge"]`);
    expect(Math.abs(badgeDrift.dy)).toBeLessThanOrEqual(0.75);
    const workingBadge = view.$(
        '[data-testid="picker"] [data-agent-id="claude"] [data-rigged-ui="badge"]',
    );
    ...
    const description = view.$(
        '[data-testid="picker"] [data-agent-id="claude"] [data-rigged-ui="mention-picker-description"]',
    );
    expect(
    ...
    const soloMeta = view.$(`${soloRow} [data-rigged-ui="mention-picker-meta"]`);
    expect(soloMeta.bounds().height).toBe(16);
    expect(soloMeta.offsets().top).toBe(14);
    expect(
        view.container.querySelector(`${soloRow} [data-rigged-ui="mention-picker-description"]`),
    ).toBeNull();
    expect(view.container.querySelector(`${soloRow} [data-rigged-ui="badge"]`)).toBeNull();
    const soloName = await view
        .$(`${soloRow} [data-rigged-ui="mention-picker-name"]`)
        .visibleMetrics();
    ...
    expect(
        view.container.querySelectorAll(
            '[data-testid="picker-empty"] [data-rigged-ui="mention-picker-row"]',
        ).length,
    ).toBe(0);
...
it("handles typing, sending, and mention picking", async () => {
    ...
    const textareaOf = (testid: string) =>
        view.$(`[data-testid="${testid}"] [data-rigged-ui="composer-textarea"]`)
            .element as HTMLTextAreaElement;
    const popoverOf = (testid: string) =>
        view.container.querySelector(
            `[data-testid="${testid}"] [data-rigged-ui="composer-popover"]`,
        );
    const rowsOf = (testid: string) =>
        Array.from(
            view.container.querySelectorAll(
                `[data-testid="${testid}"] [data-rigged-ui="mention-picker-row"]`,
            ),
        );
    const activeIdOf = (testid: string) =>
        view.container
            .querySelector(
                `[data-testid="${testid}"] [data-rigged-ui="mention-picker-row"][data-active]`,
            )
    ...
    const typingSend = view.$('[data-testid="composer-typing"] .rigged-composer__send')
        .element as HTMLButtonElement;
    ...
    expect(
        view.container.querySelector(
            '[data-testid="composer-mention"] [data-rigged-ui="mention-picker-empty"]',
        ),
    ).not.toBeNull();
    await userEvent.keyboard("{Escape}");
