...
it("holds ThreadList geometry, row anatomy, and optical alignment", async () => {
    ...
    const avatar = (id: string, nth: number) =>
        view.$(
            `[data-thread-id="${id}"] [data-rigged-ui="thread-list-avatars"] > :nth-child(${nth})`,
        );
    ...
    const more = view.$('[data-thread-id="infra"] [data-rigged-ui="thread-list-avatar-more"]');
    ...
    const soloAvatar = view.$('[data-thread-id="muted"] [data-rigged-ui="avatar"]');
    ...
    const part = (id: string, hook: string) =>
        view.$(`[data-thread-id="${id}"] [data-rigged-ui="${hook}"]`);
    ...
    const replyIcon = view.$(
        '[data-thread-id="launch"] [data-rigged-ui="thread-list-replies-icon"] svg',
    );
    ...
    const badge = (id: string) => view.$(`[data-thread-id="${id}"] [data-rigged-ui="count-badge"]`);
    ...
    expect(
        view.container.querySelector('[data-thread-id="design"] [data-rigged-ui="count-badge"]'),
    ).toBeNull();
    ...
    const followIcon = view.$('[data-thread-id="muted"] [data-rigged-ui="thread-list-follow"] svg');
    expect(followIcon.bounds()).toMatchObject({ width: 14, height: 14 });
    await ink(follow, "muted bell");
    /* Subscribed rows show no bell. */
    expect(
        view.container.querySelector(
            '[data-thread-id="launch"] [data-rigged-ui="thread-list-follow"]',
        ),
    ).toBeNull();
    ...
    expect(
        view.container.querySelector('[data-testid="empty"] [data-rigged-ui="thread-list-item"]'),
    ).toBeNull();
    const empty = view.$('[data-testid="empty"] [data-rigged-ui="thread-list-empty"]');
