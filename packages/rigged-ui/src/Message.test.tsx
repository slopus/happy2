...
it("holds Message anatomy, segment styling, and affordances", async () => {
    ...
    expect(authorMetrics.font.family).toBe("Rigged Figtree, system-ui, sans-serif");
    ...
    const time = view.$('[data-testid="m1"] [data-rigged-ui="message-time"]');
    expect(time.computedStyles(["color", "font-size", "font-weight"])).toEqual({
        color: "rgb(117, 112, 133)",
        "font-size": "11px",
        "font-weight": "500",
    });
    expect(time.textMetrics().font.family).toBe("Rigged Mono, ui-monospace, monospace");

    /* Human message has no AGENT badge; agent message shows the accent one. */
    expect(view.container.querySelector('[data-testid="m1"] [data-rigged-ui="badge"]')).toBeNull();
    const badge = view.$('[data-testid="m2"] [data-rigged-ui="badge"]');
    expect(badge.element.getAttribute("data-variant")).toBe("accent");
    expect(badge.height()).toBe(18);
    expect(view.$('[data-testid="m2"] [data-rigged-ui="badge-label"]').textMetrics().text).toBe(
        "AGENT",
    );
    /* The badge pill centers in the 20px meta row, 8px after the author. */
    const m2Meta = view.$('[data-testid="m2"] [data-rigged-ui="message-meta"]');
    expect(m2Meta.height()).toBe(20);
    expect(badge.bounds().y - m2Meta.bounds().y).toBe(1);
    const m2Author = view.$('[data-testid="m2"] [data-rigged-ui="message-author"]');
    expect(badge.bounds().x - (m2Author.bounds().x + m2Author.bounds().width)).toBeCloseTo(8, 6);
    /* Agent avatar is the rounded-square type. */
    expect(
        view.$('[data-testid="m2"] [data-rigged-ui="avatar"]').element.getAttribute("data-type"),
    ).toBe("agent");

    /* ---- Body + segments ------------------------------------------------ */

    const body = view.$('[data-testid="m1"] [data-rigged-ui="message-body"]');
    ...
    const mention = view.$('[data-testid="m1"] [data-rigged-ui="message-mention"]');
    ...
    const code = view.$('[data-testid="m1"] [data-rigged-ui="message-code"]');
    expect(code.computedStyles(["background-color", "border-radius", "font-size"])).toEqual({
        "background-color": "rgba(255, 255, 255, 0.05)",
        "border-radius": "4px",
        "font-size": "13px",
    });
    expect(code.textMetrics().font.family).toBe("Rigged Mono, ui-monospace, monospace");
    expect((await code.visibleMetrics()).pixelCount).toBeGreaterThan(0);

    const link = view.$('[data-testid="m1"] [data-rigged-ui="message-link"]');
    ...
    const reactions = view.$('[data-testid="m1"] [data-rigged-ui="message-reactions"]');
    ...
    const chips = view.container.querySelectorAll(
        '[data-testid="m1"] [data-rigged-ui="reaction-chip"]',
    );
    expect(chips.length).toBe(3);
    const chipA = view.$('[data-testid="m1"] [data-rigged-ui="reaction-chip"]:nth-of-type(1)');
    const chipB = view.$('[data-testid="m1"] [data-rigged-ui="reaction-chip"]:nth-of-type(2)');
    const chipC = view.$('[data-testid="m1"] [data-rigged-ui="reaction-chip"]:nth-of-type(3)');
    ...
    const addIcon = await view
        .$('[data-testid="m1"] [data-rigged-ui="message-react-add"] svg')
        .visibleMetrics();
    ...
    const repliesButton = view.$('[data-testid="m1"] [data-rigged-ui="message-replies"]');
    ...
    const m2Content = view.$('[data-testid="m2"] [data-rigged-ui="message-content"]');
    expect(view.$('[data-testid="attach"]').bounds().width).toBe(m2Content.bounds().width);

    /* ---- Compact follow-up (with rich segments) ---------------------------- */

    expect(view.container.querySelector('[data-testid="m3"] [data-rigged-ui="avatar"]')).toBeNull();
    expect(
        view.container.querySelector('[data-testid="m3"] [data-rigged-ui="message-meta"]'),
    ).toBeNull();
    const gutterTime = view.$('[data-testid="m3"] [data-rigged-ui="message-gutter-time"]');
    expect(gutterTime.computedStyles(["color", "font-size", "line-height"])).toEqual({
        color: "rgb(117, 112, 133)",
        "font-size": "11px",
        "line-height": "22px",
    });
    expect(gutterTime.textMetrics().font.family).toBe("Rigged Mono, ui-monospace, monospace");
    ...
    expect(view.$('[data-testid="m3"] [data-rigged-ui="message-body"]').bounds().x).toBe(68);
    expect(
        (await view.$('[data-testid="m3"] [data-rigged-ui="message-code"]').visibleMetrics())
            .pixelCount,
    ).toBeGreaterThan(0);
    expect(
        (await view.$('[data-testid="m3"] [data-rigged-ui="message-mention"]').visibleMetrics())
            .pixelCount,
    ).toBeGreaterThan(0);
...
it("centers painted ink optically in every Message text-in-a-box part", async () => {
    ...
    view.render(
        () =>
            stage(
                "o3",
                <>
                    <style>{`[data-testid="o3"] .rigged-reaction-chip { width: 44px; }`}</style>
                    <Message
                        author="Sasha K."
                        body="shipping"
                        onReactionAdd={() => {}}
    ...
    const meta = view.$('[data-testid="o1"] [data-rigged-ui="message-meta"]');
    ...
    const author = view.$('[data-testid="o1"] [data-rigged-ui="message-author"]');
    ...
    const time = view.$('[data-testid="o1"] [data-rigged-ui="message-time"]');
    ...
    const badge = view.$('[data-testid="o1"] [data-rigged-ui="badge"]');
    ...
    const gutterTime = view.$('[data-testid="o2"] [data-rigged-ui="message-gutter-time"]');
    const gutterInk = await gutterTime.visibleMetrics();
    expect(gutterInk.pixelCount, "gutter time pixels").toBeGreaterThan(0);
    const compactBody = view.$('[data-testid="o2"] [data-rigged-ui="message-body"]');
    ...
    const todayDrift = await glyphVsPill(() =>
        view.$('[data-testid="d1"] [data-rigged-ui="day-divider-label"]'),
    );
    ...
    for (const testid of ["d2", "d3"] as const) {
        const drift = await glyphVsPill(() =>
            view.$(`[data-testid="${testid}"] [data-rigged-ui="day-divider-label"]`),
        );
    ...
    const label = view.$('[data-testid="d1"] [data-rigged-ui="day-divider-label"]');
    ...
    const divider = view.$('[data-testid="d1"] [data-rigged-ui="day-divider"]');
    ...
    const addButton = view.$('[data-testid="o3"] [data-rigged-ui="message-react-add"]');
    const addIconEl = view.$('[data-testid="o3"] [data-rigged-ui="message-react-add"] svg');
...
it("anchors MessageList to the bottom and lays out sparse histories", async () => {
    ...
    const sparse = view.$('[data-testid="sparse"] [data-rigged-ui="message-list"]');
    ...
    const spacer = view.$('[data-testid="sparse"] [data-rigged-ui="message-list-spacer"]');
    expect(spacer.bounds().height).toBe(0);
    const intro = view.$('[data-testid="sparse"] [data-rigged-ui="message-list-intro"]');
    ...
    const divider = view.$('[data-testid="sparse"] [data-rigged-ui="day-divider"]');
    ...
    /* Intro typography. */
    const title = view.$('[data-testid="sparse"] [data-rigged-ui="message-list-intro-title"]');
    ...
    const description = view.$(
        '[data-testid="sparse"] [data-rigged-ui="message-list-intro-description"]',
    );
    ...
    const label = view.$('[data-testid="sparse"] [data-rigged-ui="day-divider-label"]');
    ...
    expect(label.textMetrics().font.family).toBe("Rigged Mono, ui-monospace, monospace");
    const lines = view.container.querySelectorAll(
        '[data-testid="sparse"] [data-rigged-ui="day-divider-line"]',
    );
    expect(lines.length).toBe(2);
    const line = view.$('[data-testid="sparse"] [data-rigged-ui="day-divider-line"]');
...
it("follows the newest content in MessageList unless the reader scrolled up", async () => {
    ...
    const list = view.$('[data-testid="feed"] [data-rigged-ui="message-list"]');
    ...
    expect(
        view.$('[data-testid="feed"] [data-rigged-ui="message-list-spacer"]').bounds().height,
    ).toBe(0);
