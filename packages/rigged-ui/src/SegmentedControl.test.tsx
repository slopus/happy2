...
it("holds SegmentedControl dimensions, layout, colors, and pill placement", async () => {
    ...
    for (const size of sizes) {
        ...
        expect(control.computedStyle("--rigged-segmented-count"), id).toBe("3");
        expect(control.computedStyle("--rigged-segmented-index"), id).toBe("1");
        ...
        const pill = view.$(`[data-testid="${id}"] [data-rigged-ui="segmented-control-pill"]`);
        ...
        const activeLabel = view.$(
            `[data-testid="${id}"] [data-value="week"] [data-rigged-ui="segmented-control-label"]`,
        );
        const inactiveLabel = view.$(
            `[data-testid="${id}"] [data-value="day"] [data-rigged-ui="segmented-control-label"]`,
        );
        ...
        for (const value of values) {
            ...
            const label = view
                .$(
                    `[data-testid="${id}"] [data-value="${value}"] [data-rigged-ui="segmented-control-label"]`,
                )
                .bounds();
    ...
    const contentPill = view
        .$('[data-testid="sc-content"] [data-rigged-ui="segmented-control-pill"]')
        .bounds();
...
it("holds SegmentedControl icon segments, selection sweep, fullWidth, and disabled state", async () => {
    ...
    const iconBox = view.$('[data-testid="sc-icons"] [data-value="board"] [data-rigged-ui="icon"]');
    ...
    const iconLabel = view
        .$(
            '[data-testid="sc-icons"] [data-value="board"] [data-rigged-ui="segmented-control-label"]',
        )
        .bounds();
    ...
    expect(
        view.$('[data-testid="sc-icons-sm"] [data-value="board"] [data-rigged-ui="icon"]').bounds()
            .width,
        "small icon",
    ).toBe(14);
    expect(
        view.$('[data-testid="sc-icons-lg"] [data-value="board"] [data-rigged-ui="icon"]').bounds()
            .width,
        "large icon",
    ).toBe(18);
    ...
    for (const value of ["board", "grid"]) {
        const drift = await glyphDrift(
            view,
            `[data-testid="sc-icons"] [data-value="${value}"] [data-rigged-ui="icon"]`,
        );
    ...
    for (let index = 0; index < SWEEP.length; index += 1) {
        ...
        const control = view.$(`[data-testid="${id}"]`);
        expect(control.computedStyle("--rigged-segmented-index"), id).toBe(String(index));
        const pill = view
            .$(`[data-testid="${id}"] [data-rigged-ui="segmented-control-pill"]`)
            .bounds();
        ...
        const selectedLabel = view.$(
            `[data-testid="${id}"] [data-value="${SWEEP[index]!.value}"] [data-rigged-ui="segmented-control-label"]`,
        );
        ...
        for (let other = 0; other < SWEEP.length; other += 1) {
            ...
            const otherLabel = view.$(
                `[data-testid="${id}"] [data-value="${SWEEP[other]!.value}"] [data-rigged-ui="segmented-control-label"]`,
            );
