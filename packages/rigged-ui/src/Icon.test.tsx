...
it("holds Icon box geometry across sizes", async () => {
    ...
    for (const size of SIZES) {
        const icon = view.$(`[data-testid="icon-size-${size}"] [data-rigged-ui="icon"]`);
    ...
    const fallback = view.$('[data-testid="icon-default"] [data-rigged-ui="icon"]');
    ...
    const inherit = view.$('[data-testid="icon-inherit"] [data-rigged-ui="icon"]');
    expect(inherit.computedStyles(["color", "stroke"])).toEqual({
        color: "rgb(165, 160, 176)",
        stroke: "rgb(165, 160, 176)",
    });

    const colored = view.$('[data-testid="icon-colored"] [data-rigged-ui="icon"]');
...
it("renders the entire pinned glyph set on the 20-unit grid", async () => {
    const view = createRenderer().render(
        ...
        { width: 248, height: 168, padding: 12 },
    );
    await view.ready();

    expect(iconNames.length).toBe(42);
    for (const name of iconNames) {
        ...
        expect(icon.element.getAttribute("data-rigged-ui")).toBe("icon");
    }
