...
export function AgentImageDetail(props: AgentImageDetailProps) {
    ...
    return (
        <Box
            {...rest}
            class={["rigged-agent-image-detail", local.class].filter(Boolean).join(" ")}
            data-rigged-ui="agent-image-detail"
            style={local.style}
        >
            <Show
                when={!local.error}
            ...
            >
                <Box class="rigged-agent-image-detail__status">
                    <Badge
                    ...
                    <Show when={local.status === "building"}>
                        <Box
                            ...
                            class="rigged-agent-image-detail__progress"
                            role="progressbar"
                        >
                            <span class="rigged-agent-image-detail__progress-track">
                                <span
                                    class="rigged-agent-image-detail__progress-fill"
                                    style={{ width: `${progressValue(local.progress)}%` }}
                                />
                            </span>
                            <span class="rigged-agent-image-detail__progress-value">
                ...
                <Section label="Dockerfile">
                    <Show
                        when={!local.loading}
                        fallback={
                            <p class="rigged-agent-image-detail__empty">Loading Dockerfile…</p>
                        }
                    >
                        <pre
                            class="rigged-agent-image-detail__code"
                            data-rigged-ui="agent-image-detail-dockerfile"
                        >
                            {local.dockerfile}
                        </pre>
                    </Show>
                ...
                >
                    <Show
                        ...
                        fallback={
                            <Show
                                ...
                                fallback={
                                    <p class="rigged-agent-image-detail__empty">
                                        No build output yet.
                                    </p>
                                }
                            >
                                <pre
                                    class="rigged-agent-image-detail__code"
                                    data-rigged-ui="agent-image-detail-log"
                                >
                                    {local.buildLog}
                                </pre>
                            </Show>
                    ...
                    >
                        <p class="rigged-agent-image-detail__empty">Loading build log…</p>
                    </Show>
                </Section>
            </Show>
        </Box>
...
function Section(props: { label: string; note?: string; children: JSX.Element }) {
    return (
        <section class="rigged-agent-image-detail__section">
            <header class="rigged-agent-image-detail__section-head">
                <span class="rigged-agent-image-detail__section-label">{props.label}</span>
                <Show when={props.note}>
                    <span class="rigged-agent-image-detail__section-note">{props.note}</span>
                </Show>
            </header>
            {props.children}
        </section>
