...
function MediaTile(props: { item: MediaItem; onOpen?: (id: string) => void }) {
    ...
    return (
        <button
            class="rigged-media-gallery__tile"
            data-kind={item().kind}
            data-media-id={item().id}
            data-rigged-ui="media-tile"
            onClick={() => props.onOpen?.(item().id)}
            type="button"
        >
            <span class="rigged-media-gallery__thumb" data-rigged-ui="media-thumb">
                <Show
                    when={item().thumbnailUrl}
                    fallback={
                        <span class="rigged-media-gallery__glyph" data-rigged-ui="media-glyph">
                ...
                >
                    {(url) => (
                        <img
                            alt={item().name ?? ""}
                            class="rigged-media-gallery__image"
                            data-rigged-ui="media-image"
                            src={url()}
                        />
                    )}
                </Show>
                <Show when={kindLabel()}>
                    {(label) => (
                        <span class="rigged-media-gallery__kind" data-rigged-ui="media-kind">
                ...
                <Show when={item().duration}>
                    {(duration) => (
                        <span
                            class="rigged-media-gallery__duration"
                            data-rigged-ui="media-duration"
                        >
                            {duration()}
                        </span>
                    )}
            ...
            <Show when={hasFooter()}>
                <span class="rigged-media-gallery__footer" data-rigged-ui="media-footer">
                    <Show when={item().name}>
                        {(name) => (
                            <span class="rigged-media-gallery__name" data-rigged-ui="media-name">
                                {name()}
                            </span>
                        )}
                    </Show>
                    <Show when={item().size}>
                        {(size) => (
                            <span class="rigged-media-gallery__size" data-rigged-ui="media-size">
                                {size()}
                            </span>
                        )}
                    </Show>
...
export function MediaGallery(props: MediaGalleryProps) {
    ...
    return (
        <div
            class={["rigged-media-gallery", local.class].filter(Boolean).join(" ")}
            data-rigged-ui="media-gallery"
        ...
        >
            <Show
                when={!isEmpty()}
                fallback={
                    <div class="rigged-media-gallery__empty" data-rigged-ui="media-empty">
                        {local.empty}
                    </div>
                }
            >
